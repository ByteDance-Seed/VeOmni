...
- **special-token id 不进 config.json / YAML**：boi / eoi / eos / image placeholder 等由 ``set_tokenizer()`` → module ``post_init`` 解析；FSM 只听 module emit 的 ``module_signal``，不在 YAML 硬编码 token id。
