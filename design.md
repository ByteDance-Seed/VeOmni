...
class OmniModule:
    ...
    def set_tokenizer(self, tokenizer: "PreTrainedTokenizer") -> None:
        ...
        ``model.model_path`` 在 ``OmniConfig.from_launcher()`` 时写入
        ``tokenizer_path``。
...
| **Infer overlay**（`infer_*.yaml`） | 只覆盖 `generation_graph`；deep-merge 到 train master 之上 |
...
# 指定推理场景
cfg = OmniConfig.from_launcher(
    "configs/seed_omni/janus_1.3b/veomni_janus.yaml",
    infer_type="infer_interleave",
)
```

``apply_model_path(cfg, model_path)`` 做两件事：把相对 ``weights_path`` join 到 ``model_path``；设 ``cfg.tokenizer_path = model_path``（全局 tokenizer 与 module 子目录同级，由 ``split_janus.py`` 写到 output 根）。
...
```yaml
# weights_path 相对 model.model_path；tokenizer 由 from_launcher 注入
...
├── configuration_seed_omni.py              # OmniConfig + from_yamls / from_launcher / apply_model_path（tokenizer_path 运行时注入）
...
21. **配置三层：launcher + train master + infer overlay**：launcher 管 `model_path` / 图 YAML 路径 / 训练超参；train YAML 管图词汇表与相对 `weights_path`；infer YAML 只覆盖 `generation_graph`。``OmniConfig.from_launcher()`` 解析路径并设 ``tokenizer_path``。``visualize_omni_graph.py`` 读 launcher 一次输出 ``graphs/<stem>/`` 下四张图。
