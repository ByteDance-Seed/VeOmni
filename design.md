...
class OmniModule:
    ...
    def set_tokenizer(self, tokenizer: "PreTrainedTokenizer") -> None:
        ...
        需要 vocabulary 语义的 module（``JanusTextEncoder``、backbone CFG
        钩子等）通过本方法在 build / generate 前拿到 tokenizer 引用，在
        ``post_init`` 或首次推理前解析 special-token id（boi / eoi / eos /
        image placeholder 等）——**这些 id 不写入 ``config.json``**，避免
        YAML / config 与 tokenizer 双份维护。

        OmniModel 在 init 或 ``generate()`` 入口应遍历
        ``named_omni_modules()``，对实现了本钩子的 module 注入同一份
        tokenizer。默认 no-op。
        """
...
**三层 YAML 分工**（canonical 示例：`configs/seed_omni/janus_1.3b/`）：

| 文件 | 职责 |
|------|------|
| **Launcher**（`veomni_janus.yaml`） | VeOmni 训练入口：`model.model_path`（split checkpoint 根）、`model.omni_train_yaml_path`、`model.omni_infer_yaml_path`（scenario → infer YAML）、`model.omni_infer_type`（默认推理场景）、`train.*` / `data.*` |
| **Train master**（`train.yaml`） | 图词汇表：`modules` / `nodes` / `edges` / `training_graph`。``modules.*.weights_path`` 写**相对**子目录名（如 `janus_siglip`）或绝对路径；**不写** `tokenizer_path`、**不写** special-token id |
| **Infer overlay**（`infer_*.yaml`） | 只覆盖 `generation_graph`；通过 `dict.update` 平铺覆盖 train master 的顶层键（嵌套块不能局部覆盖） |

运行时加载（训练侧由 `OmniModelArguments.load_omni_config(...)` 调用、推理侧由 `OmniInferencer(args)` 调用，两者最终都走 `OmniConfig.from_paths`）：

```python
cfg = OmniConfig.from_paths(
    model_path="/tmp/janus_1.3b_split",
    tokenizer_path="/tmp/janus_1.3b_split",
    train_yaml_path="configs/seed_omni/janus_1.3b/train.yaml",
    infer_yaml_path="configs/seed_omni/janus_1.3b/infer_interleave.yaml",  # optional
)
```

`from_paths` → `from_dict` 做两件事：把相对 `weights_path` join 到 `model_path`（绝对路径不变）；把 `tokenizer_path` 透传到 `cfg.tokenizer_path`（全局 tokenizer 与 module 子目录同级，由 `split_janus.py` 写到 output 根）。
...
| Section | 职责 |
|---|---|
| `tokenizer_path` | **运行时字段**（由 launcher ``model_path`` 注入，train YAML 不手写）。全局 tokenizer 资产目录 |
| `modules` | 模型实例池：name → `{weights_path / config_path, ...特化字段}`。**不写 model_type**（自动从 config.json 读）。**不写 begin_of_image_token_id 等 vocab id** |
...
> - **全局 tokenizer** 由 ``OmniConfig.tokenizer_path`` 指向 split checkpoint 根（与 ``janus_siglip/`` 等子目录同级）；module 通过 ``set_tokenizer()`` 拿到引用，special-token id **不落 config**；
...
>
> Janus 子类额外提供 ``emit_image_start`` / ``emit_image_end`` 两个 call-site（推理 bridge state 用），边界 token id 同样由 ``set_tokenizer()`` 解析。
>
...
# weights_path 相对 launcher 的 model.model_path；tokenizer 不在此声明
modules:
  janus_siglip:     {weights_path: janus_siglip}
  janus_vqvae:      {weights_path: janus_vqvae, freeze: true}
  janus_llama:      {weights_path: janus_llama}        # 纯 backbone，无 vocab 层
  janus_text_encoder: {weights_path: janus_text_encoder}       # wte + lm_head + Janus emit_image_*
  ...
  states:
    ...
    image_vq:
      body:
        - llama_to_vae_decode
        - vae_decode_to_llama
      token_length: {type: variable}      # 步数由 vae_decode 内部 grid 决定
      transitions:
...
| 类型 | 谁决定 | YAML | 典型场景 |
|------|--------|------|----------|
| `steps_complete` | 框架外层步数循环（`token_length: fixed`） | `{type: steps_complete}` | bridge state 跑 1 步；DiT 单次 denoise |
| `module_signal` | 模块在 return dict 写一次性 flag（`ctx_flag` 为别名） | `{type: module_signal, key: K}` | VQ 末 patch → `image_complete`；text 采样 boi/eos → `start_image_gen` / `text_done` |
...
| 策略 | 说明 | 典型场景 |
|------|------|----------|
| `variable` | 无上限，持续循环直到转移条件触发 | 文本 AR（`module_signal(text_done)` 转 done）、audio stream；**VQ 图像**（`module_signal(image_complete)`） |
| `fixed` | 静态固定步数 | DiT 单次调用（`value: 1`）；Janus bridge state（`emit_image_*` 后跑 1 步 AR） |
...
stateDiagram-v2
    ...
    image_vq_start --> image_vq : steps_complete
    ...
    image_vq_end --> text_ar : steps_complete
...
* `text_encoder` 挂 `tok_encode`（**chat-template + tokenize + wte**：从 `conversation_list` 产出 flat `input_ids` / `labels` / `attention_mask`，**并按模态 split 出新 `conversation_list`**）和 `tok_decode`（训练算 `_loss`、推理 hidden→sample→next token + **`module_signal`**）。推理-only 节点 `emit_image_start` / `emit_image_end` 由 `JanusTextEncoder` 提供，边界 id 由 `set_tokenizer()` 解析。

`janus_llama` 自身不再持有 `wte` / `lm_head`——就是个纯 backbone（`inputs_embeds → hidden_states`）。

`scripts/multimodal/convert_model/split_janus.py` 把原始 Janus checkpoint 拆成 4 份 module 子目录 + **根目录全局 tokenizer**：`janus_siglip/`、`janus_vqvae/`、`text_encoder/`、`janus_llama/`，以及 `tokenizer.json` 等（与 subfolder 同级）。YAML 里 ``weights_path`` 写相对名（如 `janus_siglip`），launcher 的 ``model.model_path`` 指向 split 根。

```yaml
# weights_path 相对 model.model_path；tokenizer 由 from_paths 透传

modules:
  janus_siglip:     {weights_path: janus_siglip}
  janus_vqvae:      {weights_path: janus_vqvae, freeze: true}
  janus_text_encoder: {weights_path: janus_text_encoder}          # wte + lm_head + emit_image_*
  janus_llama:      {weights_path: janus_llama}               # 纯 backbone，无 vocab 层
...
| 类别 | 是否全局 | 存放位置 |
|------|---------|---------|
| **tokenizer** | **全局**（一个 Omni 模型一份） | split / save 时在 **checkpoint 根目录**（``tokenizer.json`` 与 ``janus_siglip/`` 等同级）。``OmniConfig.tokenizer_path`` 运行时设为 ``model.model_path``；**不在 train YAML 手写**。Module 通过 ``set_tokenizer()`` 共享引用 |
...
- **special-token id 不进 config.json / YAML**：boi / eoi / eos / image placeholder 等由 ``set_tokenizer()`` → module ``post_init`` 解析；FSM 听 ``module_signal`` 而非 ``token_match(id)``。
...
├── configuration_seed_omni.py              # OmniConfig + from_paths / from_dict（model_path 解析相对 weights_path，tokenizer_path 透传）
...
└── modules/                                # 每个子模块一个文件夹；文件名固定 configuration / modeling / processing
    ...
    │   │   └── modeling.py                  # TextEncoder —— 默认实现：按 conversation_list 拼接文本、tokenize、产生 input_ids/labels、按模态 split 出新 conversation_list（value=inputs_embeds）；family-specific 子类 override chat-template + set_tokenizer 解析 special-token id
    ...
    │   │   ├── modeling.py
    │   │   └── processing.py
    │   └── text_encoder/                      # JanusTextEncoder：decode 写 module_signal；emit_image_start/end
    │       ├── configuration.py               # 不含 begin_of_image_token_id 等（runtime via set_tokenizer）
    │       └── modeling.py
...
14. **生命周期分层**：weights 加载走 `build_foundation_model` + `build_parallelize_model`（多 path 扩展）；保存由每个 module 自己的 `CheckpointCallback` 写到 subfolder（自带 config + 可选 processor asset）；**全局 tokenizer** 写在 checkpoint 根、``OmniConfig.tokenizer_path = model_path``（launcher 注入，train YAML 不手写）。Special-token id 不进 ``config.json``——``set_tokenizer()`` 运行时解析。
...
20. **FSM 转移：`module_signal` 优先于 `token_match`**：模块在 return dict 写语义化一次性 flag（`image_complete`、`start_image_gen`、`text_done` …），YAML 用 `{type: module_signal, key: K}`；框架 pop key 防 stale。`steps_complete` 留给固定步数 state。`token_match` 仅旧图兼容；新 Janus 图不在 YAML 硬编码 vocab id。

21. **配置三层：launcher + train master + infer overlay**：launcher 管 `model_path` / 图 YAML 路径 / 训练超参；train YAML 管图词汇表与相对 `weights_path`；infer YAML 平铺覆盖 `generation_graph`。`OmniConfig.from_paths(model_path, tokenizer_path, train_yaml_path, infer_yaml_path=None)` 解析路径并透传 `tokenizer_path`。`visualize_omni_graph.py` 读 launcher 一次输出 `graphs/<stem>/` 下四张图。
