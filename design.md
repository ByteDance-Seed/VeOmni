...
class OmniModule:
    ...
    def forward(self, **kwargs) -> Dict[str, Any]:
        ...
        ``conversation_list`` 加 boundary marker、text_encoder 想从
    ...
    def set_tokenizer(self, tokenizer: "PreTrainedTokenizer") -> None:
        """
        Optional. tokenizer 在 V2 是 per-module asset（住在 text_encoder module
        的 subdir）；大多数情况下每个 module 自带 / 不需要 tokenizer。这个钩子
        仅用于**少数需要共享 tokenizer 引用**的场景：
          - 一个 ViT 模块需要懂 boi/eoi token 字符串，但不想自己再带一份
            tokenizer，可在 OmniModel.__init__ 阶段通过本钩子拿到 text_encoder
            的 tokenizer 引用，仅用于 ``convert_tokens_to_ids`` 之类的查询；
          - CFG ``build_cfg_uncond_inputs`` 取 ``pad_token_id`` 时，backbone
            可通过本钩子借用 text_encoder 的 tokenizer。
        OmniModel 在 init 后由 text_encoder 模块（如果存在）发起一次广播：把
        text_encoder.tokenizer 注入给所有实现了 ``set_tokenizer`` 的 sibling
        module。默认 no-op。
        """
...
class JanusTextEncoder(TextEncoder, OmniModule):
    """Janus 的 chat-template + tokenizer + wte + lm_head 一体模块。
    继承 base/text_encoder/TextEncoder（提供拼接 + tokenize + wte 的默认实现），
    自带 Janus tokenizer asset，重写 chat-template 拼接逻辑。
    encode: conversation_list → input_ids + labels + attention_mask
            + 新 conversation_list（按模态 split，value=inputs_embeds）
    decode: hidden_states → logits → sample / lm_loss"""
```
...
> **`text_encoder`：model-specific 的 chat-template + tokenizer + wte + lm_head 模块。** 这一层是 V2 数据流的核心枢纽：
> - **自带 tokenizer**（住在 `modules/<family>/text_encoder/tokenizer/` 子目录）；
...
>
> 跟 V1 的"通用 wte + lm_head"不同，V2 的 `text_encoder` 是 **model-specific**——每个 family 一份 `modules/<family>/text_encoder/`，因为 chat template 拼接逻辑、特殊 token 集合、is system prompt 处理都是 family 相关的。`scripts/split_<family>.py` 会自动把 HF model 的 `embed_tokens` + `lm_head` + `tokenizer` 一并拆到 `text_encoder/` 子目录。

```yaml
# ── 模块注册表（不写 model_type，HF AutoConfig 自动读）──────────────
# tokenizer 不在顶层，跟随 janus_text_encoder module 的 weights subdir
modules:
  ...
  janus_text_encoder: {weights_path: /path/to/janus_text_encoder}   # chat-template + tokenizer + wte + lm_head
...
```mermaid
flowchart TD
    subgraph raw_batch [raw_batch — 所有 node 全局透明可见]
    ...
    subgraph exec ["OmniModel.forward — training_graph.edges 拓扑一次遍历"]
        ...
        tok_e["tok_encode<br/><i>janus_text_encoder.encode</i><br/>← conversation_list (含 boundary markers)<br/>→ {conversation_list (split, value=inputs_embeds), input_ids, labels, attention_mask}"]
        ...
        tok_d["tok_decode<br/><i>janus_text_encoder.decode</i><br/>← hidden_states<br/>← labels (raw_batch)<br/>→ {lm_loss}  scalar, post_forward 内 token-mean"]
        vae_d["vae_decode<br/><i>janus_vqvae.decode</i><br/>← hidden_states<br/>← gt_token_ids (vae_encode)<br/>→ {gen_loss}  scalar, post_forward 内 token-mean"]
        endN((end))
    end

    raw_batch --> vit & vae_e
    vit -.modify.-> raw_batch
    vae_e -.modify.-> raw_batch
    raw_batch --> tok_e
...
**Forward queue 由 topo sort 自动推导**——`scripts/visualize_omni_graph.py` 会基于 `training_graph.edges` 跑 Kahn topo sort，输出严格的执行序列。注意 **`tok_encode` 必须等 `vit_encode` / `vae_encode` 完成**——它们对 `conversation_list` 的修改（插入 boi/eoi marker item）是 `tok_encode` 拼接 chat template 时的必要前置：
...
generation_graph:
  ...
  states:
    ...
    # 路由回 janus_llama 作为下一步 inputs_embeds。VQ 解码到最后一格时
    # vae_decode 主动写 ctx['image_complete']=True 触发转移。
    image_vq:
      body:
        - llama_to_vae_decode    # janus_llama 执行 → hidden_states
        - vae_decode_to_llama    # vae_decode 执行 → embed 路由回 janus_llama
      token_length: {type: variable}      # 实际步数由 vae_decode 主动 signal
      transitions:
        - {condition: {type: ctx_flag, key: image_complete}, next_state: text_ar}
...
| 策略 | 说明 | 典型场景 |
|------|------|----------|
| `variable` | 无上限，持续循环直到转移条件触发 | 文本 AR（EOS 触发转 done）、audio stream；**VQ 图像**（module 主动 signal 完成，配 `ctx_flag` 转移条件） |
| `fixed` | 静态固定步数 | DiT 单次调用（`value: 1`），跑完整去噪循环 |
| `from_request` | 从用户的推理请求参数读取 | 用户指定分辨率 → 对应 token 数（少数显式 budget 场景） |
| `from_generated_text` | 调用模块内定义的解析函数，从已生成内容提取 | AR 先生成 `<image w=1024 h=1024>`，再据此决定后续步长 |

> **关于 `variable` + `ctx_flag` 的 module-driven completion 模式**：VQ 图像、audio EOS、video frame end 这种"由模块决定何时停"的场景，标准做法是 `token_length: {type: variable}` 配 `transitions: [{condition: {type: ctx_flag, key: <K>}, next_state: ...}]`。模块在最后一步把 `ctx[<K>] = True` 写进上下文，FSM 检测到 truthy 值后自动触发转移并清掉这个 key（避免下一个 state 误触发）。**不要**用 `fixed: 576` 之类的硬编码 patch budget——VQ patch 数应该由模型自带的 grid 算术（例如 24×24=576）决定，不应该污染到 YAML 层。
...
stateDiagram-v2
    ...
    image_vq : image_vq\nbody: janus_llama→vae_decode (反馈循环)\ntoken_length: variable + ctx_flag(image_complete)
    image_vq --> text_ar : ctx_flag image_complete
...
* `janus_text_encoder` 挂 `tok_encode`（**chat-template + tokenize + wte 一气呵成**：从 `conversation_list` 产出 flat `input_ids` / `labels` / `attention_mask`，**并按模态 split 出新 `conversation_list`**——每个 segment 的 value 是该段的 `inputs_embeds` 张量；image/audio/video segment value 是 1 个 placeholder embedding，等 backbone splice 时替换成 N patch tokens）和 `tok_decode`（**统一 text head**——训练算 `lm_loss`、推理 hidden→sample→next token）。`tie_word_embeddings=true` 时 encode/decode 共用 `embed_tokens.weight`，否则各持一份矩阵。**tokenizer 作为 asset 跟随 `janus_text_encoder/tokenizer/`**。

`janus_llama` 自身不再持有 `wte` / `lm_head`——就是个纯 backbone（`inputs_embeds → hidden_states`）。

`scripts/split_janus.py` 把原始 Janus checkpoint 拆成 4 份：`janus_siglip/`、`janus_vqvae/`、`janus_text_encoder/`（含 tokenizer asset）、`janus_llama/`，每份带 config.json（`model_type` 已写入）。YAML 里只填 `weights_path` 即可，model_type 自动从 config 读。

```yaml
# tokenizer 跟随 janus_text_encoder module，不在顶层
...
generation_graph:
  ...
  states:
    text_ar:
      ...
      # tok_decode 把 next id 通过 tok_decode_to_input 反馈给 tok_encode，
      # tok_encode 把它附加到 conversation_list 末尾的 assistant text item，
      # 下一步重新拼接 chat template（KV cache 由 backbone 复用）。
    ...
    image_vq:
      ...
      # vae_decode 解码到最后一格 patch 时主动写 ctx['image_complete']=True 触发转移。
...
modules:
  ...
  thinker_text_encoder:  {weights_path: /path/to/thinker_text_encoder}        # thinker 的 chat-template + tokenizer + wte + lm_head
  thinker_llm:         {weights_path: /path/to/thinker_llm}               # 纯 backbone
  talker_text_encoder:   {weights_path: /path/to/talker_text_encoder}         # talker 的 chat-template + tokenizer + wte + lm_head
...
modules:
  ...
  bagel_text_encoder: {weights_path: /path/to/bagel_text_encoder}    # chat-template + tokenizer + wte + lm_head
...
│   ├── model.safetensors
│   └── preprocessor_config.json      # 此模块的 asset（image processor）
├── janus_vqvae/
│   ├── config.json
│   ├── model.safetensors
│   └── preprocessor_config.json      # 此模块的 asset（image processor）
├── janus_llama/
│   ├── config.json                   # 纯 backbone，无 asset
│   └── model.safetensors
├── janus_text_encoder/                  # chat-template + tokenizer + wte + lm_head
│   ├── config.json
│   ├── model.safetensors
│   └── tokenizer/                     # 此模块的 asset（HF tokenizer）
│       ├── tokenizer.json
│       ├── special_tokens_map.json
│       └── tokenizer_config.json
└── omni_config.yaml                   # 顶层 OmniConfig（modules / nodes / edges / graphs）
                                       # 注意：无顶层 tokenizer_path 字段
```
...
| 类别 | 是否全局 | 存放位置 |
|------|---------|---------|
| **tokenizer** | **per-module**（住在 text_encoder module 的 subdir） | 由 `text_encoder` module 自带（如 `modules/janus/text_encoder/tokenizer/`）。OmniConfig **顶层不再有** `tokenizer_path`。如果一个模型有多个 text encoder 或场景需要多 tokenizer，每个 text-side module 各自带一份。纯 DiT 模型（无 text encoder）配置里完全不出现 tokenizer，框架不强求。 |
...
| 任何其他 processor（video frame extractor / chat template 拼接 helper / ...） | per-module | 跟随该 module subdir。chat template 不是独立 asset，它是 text_encoder 模块的 forward 实现细节（用 Python 代码或 jinja 模板都可以）。 |

- **每个 module 0 或 1 个 asset，不会重复**——一个 module 只对应一种 processor / tokenizer。
- **tokenizer 不再是顶层全局**：raw data 进入 OmniModel 时只是 conversation_list（list[dict]），还没 tokenize；tokenize 发生在 text_encoder.forward 内部（数据流 Layer 5）。这条契约让：
  - **emit 边界 token / chat template 拼接 / labels 计算 / 构造 CFG uncond 分支** 全部封进 text_encoder 或对应模态的 encoder—— OmniModel 框架不感知任何 model-specific 的 token id；
  - **pure DiT 不需要 tokenizer**：纯 DiT 配置里完全不写 text_encoder module，raw_batch 起点是空 conversation_list 或者纯 image conversation；
...
│       - 末尾加 EOS                                                                     │
│    2. 算 labels（image/audio 段对应位置填 -100；text 段按 from_assistant + loss_mask） │
│    3. 算 attention_mask                                                                │
...
│       [{type:"text",  value:<Tensor(L_text1, D)>, from_assistant:False, loss_mask:0},  │
│        {type:"image", value:<Tensor(1, D)>,        from_assistant:False, loss_mask:0}, │
│        {type:"boi",   value:<Tensor(1, D)>,        from_assistant:False, loss_mask:0}, │
│        ...]                                                                            │
...
> - segment 的 from_assistant / loss_mask 字段保留——backbone splice 同步 labels 时直接读 segment 元信息；
...
class JanusLlama(LlamaModel, OmniModule):
    def pre_forward(
        self,
        conversation_list,                   # 按模态 split 的 list of dict，每个 item 含 type/value/from_assistant/loss_mask
        ...
        **_,
        ...
        # labels / attention_mask 同步用 segment 元信息（type / from_assistant / loss_mask）
...
| **text_encoder**（base 在 `modules/base/text_encoder/`，family 在 `modules/<family>/text_encoder/` 继承之）| (1) 自带 tokenizer asset；(2) 接受已经被 vision/audio module 修改过的 conversation_list；(3) 按本 family 的 chat template 规则拼接 token_id 序列（含 system prompt / EOS / boi-eoi 等 marker token；image / audio / video / vq_image item 用 1 个 placeholder token id 占位）；(4) 算 flat labels（image/audio 段填 -100，text 段按 from_assistant + loss_mask）；(5) 算 flat attention_mask；(6) 过 wte → inputs_embeds（flat）；(7) **按模态 split** 输出新 conversation_list（item.value=该段 inputs_embeds segment）；(8) SP slice sequence-domain tensors |
...
└── modules/                                # 每个子模块一个文件夹；文件名固定 configuration / modeling / processing
    ...
    │   ├── text_encoder/                     # 通用 text_encoder 基类（chat-template + tokenize + wte + lm_head）
    │   │   ├── configuration.py             # TextEncoderConfig
    │   │   └── modeling.py                  # TextEncoder —— 默认实现：按 conversation_list 拼接文本、tokenize、产生 input_ids/labels、按模态 split 出新 conversation_list（value=inputs_embeds）；family-specific 子类只需 override chat-template 拼接逻辑 + 自带 tokenizer asset
    ...
    │   │   ├── modeling.py
    │   │   └── processing.py
    │   └── text_encoder/                      # JanusTextEncoder（继承 TextEncoder + Janus 自带 chat-template/tokenizer + boundary marker emit）
    │       ├── configuration.py
    │       ├── modeling.py
    │       └── tokenizer/                    # 此模块的 asset（HF tokenizer，跟随 weights subdir）
...
1. **模块按 model family 组织**：`modules/<family>/` 下放该 family 拆出的子模型；`modules/base/` 放跨 family 复用的小模块（通用 text_encoder、MLP adapter 等）。代码层面所有模块都是 OmniModule mixin 形态——HF / diffusers 模型 + OmniModule 多继承。
...
14. **生命周期分层**：weights 加载走 `build_foundation_model` + `build_parallelize_model`（多 path 扩展）；保存由每个 module 自己的 `CheckpointCallback` 写到 subfolder（自带 config + 可选 asset，**含 tokenizer**）；OmniConfig 顶层不再有全局 `tokenizer_path` 字段——tokenizer 是 text_encoder module 的私有 asset。
