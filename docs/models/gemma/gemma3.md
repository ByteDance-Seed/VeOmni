...
A text-only Gemma3 conversation-training recipe.

## 2. Variants and recipes

The primary checkpoint in this walkthrough is [google/gemma-3-270m](https://huggingface.co/google/gemma-3-270m).
Choose the configuration matching your task:

| Configuration | Input data |
| --- | --- |
| [gemma3.yaml](../../../configs/text/gemma3.yaml) | conversation |
...
```bash
bash train.sh tasks/train_text.py configs/text/gemma3.yaml \
  --model.model_path downloads/gemma-3-270m \
  --data.train_path /path/to/prepared-data \
  --train.checkpoint.output_dir outputs/gemma3 \
...
The checked-in checkpoint is the 270M text model. This recipe does not establish Gemma vision training support. It selects FlexAttention and Ulysses SP=2, and disables dynamic batching. Match the accelerator count and attention backend to those settings.
