...
Config dir: `configs/seed_omni/janus_1.3b/`

| File | Role |
|------|------|
| `veomni_janus.yaml` | Top-level launcher: model paths, data, FSDP/optimizer, checkpoint, wandb. |
| `train.yaml` | Per-module training graph (which module runs at each node + freeze/LoRA). |
| `data.yaml` | Weighted multisource data list (ImageNet + ShareGPT4V). |
| `infer_und.yaml` / `infer_gen.yaml` / `infer_interleave.yaml` | Per-type generation graphs. |
...
The `output_dir` becomes `model.model_path` in `veomni_janus.yaml`.
...
```yaml
# configs/seed_omni/janus_1.3b/data.yaml
sources:
...
`--model.omni_infer_type`. Point `--infer.model_path` at a **split-checkpoint
root** that holds one subfolder per module (`janus_siglip/`, `janus_vqvae/`,
`janus_text_encoder/`, `janus_llama/`), each with its own `config.json` +
weights. The step-1 converter output already has this layout, so you can infer
directly with the converted base model.
...
**Image understanding (I2T / VQA)** — `infer_und.yaml`:
...
**Text-to-image (T2I)** — `infer_gen.yaml` (`guidance_scale` enables CFG):
...
**Interleaved** — `infer_interleave.yaml` (default `omni_infer_type`) mixes text
and image generation in one graph.
