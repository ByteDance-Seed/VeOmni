...
| `test_kernel_registry_numerical.py` | Numerical alignment per (op, variant, impl) | CUDA; the FlashQLA `chunk_gated_delta_rule` case skips unless running on SM90 exactly (Hopper) — the `flash-qla` package itself now rides along inside the `gpu` extra. SM10x is WIP upstream and won't work yet. |
