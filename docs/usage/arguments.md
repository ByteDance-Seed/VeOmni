...
| activation_offload_modules | `List[str]` | `[]` | Module name patterns for async offload. Supports segment-aware glob (`model.layers.*` matches direct children only) and `{*}` for sequential groups (`model.layers.{*}`). |
