...
| dsa_indexer_loss | `bool` | `False` | Train the DeepSeek sparse attention Lightning Indexer with the DeepSeek-V3.2 eq. (4) sparse KL objective. Requires `dsa_indexer_implementation: tilelang` and `dsa_attention_implementation: tilelang`, both refused at launch, and `ulysses_size: 1` with `cp_size: 1`, refused on the first forward. Nothing here is silently ignored. Only DeepSeek-V4 implements it — any other model type refuses the flag at model-build time. See the section below. |
