...
**Defaults are GPU-optimal** (Liger / Triton). On Ascend NPU,
...
| moe_implementation | `str` | `"triton"` | MoE experts forward implementation. `triton` uses Triton group-gemm (GPU SM70+ or MLU); `quack` uses Quack CUTLASS/CuTe (GPU, SM90+); `npu` uses the NPU group-gemm kernel; `mlu` uses Apex grouped-GEMM on MLU; `eager` is the reference loop. A value still equal to the GPU default auto-resolves to `npu` on NPU; explicit incompatible non-default overrides raise. |
