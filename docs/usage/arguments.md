...
| gdn_context_parallel_implementation | `Literal["disabled", "state_passing_lossless", "kcp"]` | `"disabled"` | Gated DeltaNet context-parallel algorithm. `state_passing_lossless` uses native-chunk ownership, reversible all-to-all, and autograd-aware recurrent-state/halo P2P; it is supported on GPU and NPU. `kcp` keeps the same lossless ownership/halo layout but replaces recurrent-state P2P with the fixed-size `ttx_bc8_m1` affine-prefix collective and is currently Ascend-NPU-only. Either enabled mode requires the Qwen3.5 dense/MoE packed-text contract described under `cp_size`; use `disabled` when `cp_size=1`. |
...
| cp_size | `int` | `1` | Context-parallel size. Production `cp_size>1` is currently limited to Qwen3.5 dense/MoE text-only packed causal-LM training with `train.dyn_bsz=True`, a VeOmni FlashAttention 2/3/4 SP backend, zero attention dropout, no sliding-window attention, and `gdn_context_parallel_implementation` set to `state_passing_lossless` or `kcp`. The size must be a power of two. Generic Ring CP is not yet a production configuration; `cp_size=1` requires the selector to remain `disabled`. |
