...
| stage_dir | `Optional[str]` | `None` | Write checkpoints here and copy them to `output_dir` afterwards, instead of writing straight to `output_dir`. For a destination far slower than local disk, where a direct write can block the training loop long enough to trip the collective timeout. Nothing is probed and free space is not checked — point it at a node-local filesystem that can hold every rank on the node writing the model plus its optimizer state at once. Cannot be combined with `save_async`. |
| stage_promote_async | `bool` | `False` | With `stage_dir`, resume training once the checkpoint is staged and copy it to `output_dir` on a background thread. The next save, a load, and the end of training all wait for that copy. Keeps the staged checkpoint on the scratch disk until the copy ends. Requires `stage_dir`. |
| stage_promote_timeout_seconds | `Optional[int]` | `None` | Collective timeout in seconds for the background promotion's own process group, a positive integer. Must outlast the copy — the ranks not copying wait on it for the whole duration. `None` keeps gloo's 30-minute default. |
