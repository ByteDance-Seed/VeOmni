...
| gdn_context_parallel_implementation | `Literal["disabled", "state_passing_lossless", "kcp"]` | `"disabled"` | Gated DeltaNet context-parallel algorithm. `state_passing_lossless` uses native-chunk ownership, reversible all-to-all, and autograd-aware recurrent-state/halo P2P; it is currently Ascend-NPU-only. `kcp` keeps the same lossless ownership/halo layout but replaces recurrent-state P2P with the fixed-size `ttx_bc8_m1` affine-prefix collective and is also Ascend-NPU-only. Either enabled mode requires the Qwen3.5 dense/MoE packed-text contract described under `cp_size`; use `disabled` when `cp_size=1`. |
