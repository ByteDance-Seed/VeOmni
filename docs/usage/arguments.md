...
| muon_head_split_exclude_indexer | `bool` | `True` | Keep a *known* DSA indexer's projections on full-matrix Muon even when `muon_head_split_modules` names one of them (DeepSeek-V4's indexer and its MLA both use `q_b_proj`). Recognized by class via `_DSA_INDEXER_CLASS_NAMES` in `veomni/optim/muon.py`, which currently lists DeepSeek-V4's only; an unlisted indexer is head-split as named, silently. Set `False` to head-split the indexer too. |
