...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| attn_implementation | `Optional[Literal[...]]` | `"flash_attention_2"` | Attention implementation. Supported public values include `eager`, `sdpa`, `flash_attention_2/3/4`, `flex_attention`, `aiter`, `magi_attention`, and `native-sparse`. Under the VeOmni modeling backend, Flash, Flex and aiter values resolve to SP-aware registry names. FlexAttention requires a model-provided native `BlockMask`; Ulysses currently requires it to be head-broadcast. `aiter` needs AMD ROCm plus the `aiter` package — see [VeOmni on AMD ROCm](../hardware_support/rocm/README.md). |
