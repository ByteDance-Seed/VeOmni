...
| muon_head_split_modules | `List[str]` | `[]` | Leaf module names to head-split, e.g. `[q_b_proj]` in YAML, `--train.optimizer.muon_head_split_modules q_b_proj` on the command line (no brackets). No default — required when `muon_head_group_size >= 1`; a list matching no module raises instead of falling back to full-matrix Muon. |
