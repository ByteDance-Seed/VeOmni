...
| gdn_context_parallel_implementation | `Literal["disabled", "state_passing_lossless", "kcp"]` | `"disabled"` | Context-parallel selector. `disabled` is the backwards-compatible generic Ring/Hybrid CP path for non-GDN causal models. `state_passing_lossless` uses native-chunk ownership, reversible all-to-all, and autograd-aware recurrent-state/halo P2P; it is currently supported on Ascend NPU. `kcp` keeps the same lossless ownership/halo layout but replaces recurrent-state P2P with the fixed-size `ttx_bc8_m1` affine-prefix collective and is currently Ascend-NPU-only. Qwen3.5 must select an explicit lossless mode; it never silently falls back to generic Ring. |
...
| cp_size | `int` | `1` | Context-parallel size. For non-GDN causal models, `cp_size>1` uses generic Ring/Hybrid CP with the existing packed dynamic-batch, power-of-two, FlashAttention-2/3/4, zero-dropout, and no-sliding-window contract. Qwen3.5 dense/MoE text-only packed causal-LM training additionally requires `gdn_context_parallel_implementation` set to `state_passing_lossless` or `kcp`; it never falls back to generic Ring. The size must be a power of two. |
