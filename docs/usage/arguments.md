...
| muon_head_split_exclude_indexer | `bool` | `True` | Keep a DSA indexer's projections on full-matrix Muon even when `muon_head_split_modules` names one of them (DeepSeek-V4's indexer and its MLA both use `q_b_proj`). Set `False` to head-split the indexer too. |
