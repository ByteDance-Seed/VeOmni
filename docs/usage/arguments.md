...
| save_async | `bool` | `False` | Return before the checkpoint is durable at `output_dir`, leaving the slow part to a background thread. Without `stage_dir` that is DCP's own async write; with it, the staging write stays inline and the copy to `output_dir` runs in the background. The next save, a load and the end of training all wait for it. |
| stage_dir | `Optional[str]` | `None` | Write checkpoints here and copy them to `output_dir` afterwards, instead of writing straight to `output_dir`. For a destination far slower than local disk, where a direct write can block the training loop long enough to trip the collective timeout. Nothing is probed and free space is not checked — point it at a node-local filesystem that can hold every rank on the node writing the model plus its optimizer state at once. With `save_async` the copy to `output_dir` runs in the background, so the staged checkpoint stays on the scratch disk until it finishes. |
| save_async_timeout_seconds | `Optional[int]` | `None` | Collective timeout in seconds for the process group a `save_async` checkpoint runs on, a positive integer. Must outlast the work — the ranks not writing wait on it for the whole duration. `None` keeps gloo's 30-minute default. |
