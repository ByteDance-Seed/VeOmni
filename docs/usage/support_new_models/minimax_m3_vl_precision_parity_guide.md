...
- forward: `loss`、`logits`、`image_hidden_states`、`video_hidden_states`、direct `MiniMaxM3VLMultiModalProjector` hook outputs；
...
目标机器操作者可以直接按 [minimax_m3_vl_target_machine_runbook.md](./minimax_m3_vl_target_machine_runbook.md) 执行；该 runbook 包含 checkpoint 下载、NPU runtime gate、full preflight、full forward、多卡、最终 audit 和 artifact 打包回传步骤。本节保留各 gate 的单独命令和判定标准。
...
如果要直接查看原始目标逐项完成度，运行 objective audit。当前 PR 在没有 full checkpoint 和多卡 target-machine artifact 时应该输出 `objective_complete=false`，这不是失败，而是防止把 toy/SFT smoke 误报成最终精度完成：

```bash
python scripts/multimodal/audit_minimax_m3_vl_precision_objective.py \
  --output-json docs/usage/support_new_models/artifacts/minimax_m3_vl_precision_parity/precision_objective_audit.json
```
...
注意：现有 NPU parity artifacts 是较早生成的 38-check 证据，覆盖 forward、router、backward 和 AdamW tolerance，但不包含新增的 direct projector hook checks。目标机按本手册使用最新脚本复跑时，`audit_minimax_m3_vl_target_artifact_bundle.py --require-target-toy` 会要求 NPU target toy artifact 包含 `projector.record_count`、`projector.0.output` 和 `projector.1.output`。
