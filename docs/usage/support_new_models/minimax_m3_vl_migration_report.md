...
Assessment: the current MiniMax M3 VL PR is a useful patchgen-generated migration slice and has the right file families for a VeOmni new-model PR. It is not a completed production model migration yet, because completion still requires full public checkpoint forward parity, public-checkpoint trainer smoke, GPU parity reruns on target machines, and SP/EP e2e alignment on multi-card hardware. Synthetic image/video `VLMTrainer` forward/backward evidence, data-transform-level path/bytes video-container evidence, public checkpoint index/state coverage, public shard-header shape coverage, remote real-tensor payload sampling, CPU and single-card Ascend NPU official MiniMax loader vs VeOmni toy precision parity, real checkpoint full-forward tooling, and single-card Ascend NPU generated-model SFT loss evidence are present and documented below.
...
- MiniMax GitHub release repo: `MiniMax-AI/MiniMax-M3`, main `a4ee8150dd58920c6f87c6aba26e3b6cdea56d75`
- MiniMax GitHub repo contents: README/figures release notes only; executable config/processor/checkpoint source is the official HF repo
...
- `transformers/v5.12.0` MiniMax modeling: 200; used as loader for the official MiniMax HF config/checkpoint

VeOmni global `transformers-stable` pin is unchanged. MiniMax example docs require a local `transformers>=5.12.0` environment.

Source recheck: HF sha has moved to `b1c79b9c07578aeebf33c2aeff0f6de8a96b02b1` with `lastModified=2026-06-22T06:35:16.000Z`; the public safetensors index is still HTTP `200` with `23416` weight-map keys and `59` shard filenames. Projector index coverage is split across `multi_modal_projector.linear_{1,2}.{weight,bias}` and `patch_merge_mlp.linear_{1,2}.{weight,bias}`; the latter maps to transformers generated `merge_linear_{1,2}`. Precision reference should be understood as the official MiniMax HF config/processor/checkpoint loaded by a compatible Transformers MiniMax loader, not as Transformers repository code alone.
...
Additional local checks for the multi-card artifact audit gate:
...
passed=true, full_checkpoint_forward_passed=false, multicard_passed=false
...
scripts/multimodal/run_minimax_m3_vl_full_checkpoint_parity.sh \
  ...
  --preflight-only
expected failure on this host because the system Python lacks torch/transformers; the preflight artifact still proves local checkpoint index/shard/config detection and records failed MiniMax reference loader / VeOmni candidate / converter entrypoint imports.
...
Official MiniMax loader vs VeOmni generated toy precision parity:
...
The script reads local public safetensors payloads or remote safetensors tensor byte ranges, applies `MiniMaxM3VLCheckpointTensorConverter` to real tensors, compares converted key/shape/dtype metadata against the VeOmni generated model state, and records value fingerprints for sampled payload evidence. The remote sample reads 11 real tensor payloads from Hugging Face `model-00001/00003/00026/00059-of-00059.safetensors` via HTTP Range (`payload_bytes_read=55808`), maps language dense/sparse attention norms, a MoE correction bias, vision tower tensors, multi-modal projector bias, and patch-merge projector bias into generated VeOmni state keys, and passes with `shape_mismatch_count=0`, `missing_model_key_count=0`, and only the expected `F32->BF16` runtime cast for the correction bias. It also runs sampled state-load cast/copy verification against generated model state metadata: `sampled_state_load.passed=true`, `loaded_tensor_count=11`, `dtype_cast_groups={\"BF16->BF16\": 10, \"F32->BF16\": 1}`, and `value_mismatch_count=0`. With `--mode forward --confirm-full-load`, the same script now runs sequentially: it loads the complete local checkpoint into the upstream transformers reference, records fixed-prompt logits/top-k/greedy baseline, releases the HF model, then streams converted public checkpoint tensors into the VeOmni generated model without materializing a full converted tensor dict. `run_minimax_m3_vl_full_checkpoint_parity.sh` wraps that full-forward command and immediately runs the artifact auditor in `--require-full-checkpoint-forward` mode, so target machines get one command that produces both the forward artifact and strict audit artifact. The toy checkpoint forward smoke uses `torch_dtype=float32`, runs a mixed image+video prompt, and passes with top-level `full_checkpoint_load_executed=true`, `reference_device=cpu`, `candidate_device=cpu`, `num_checks=12`, `failed=[]`, `streaming_model_load=true`, strict missing/unexpected key counts of `0`, matching input ids/mask/position/grid/pixels, `forward.logits`, `forward.image_hidden_states`, and `forward.video_hidden_states` max diff `0.0`, equal top-k ids, and equal greedy ids. The same toy checkpoint forward runner also passes with `reference_device=cpu`, `candidate_device=npu:0` in the Ascend container, proving the full-checkpoint runner can compare HF CPU reference against VeOmni NPU candidate; that artifact records `tolerances.forward={\"atol\": 5e-4, \"rtol\": 5e-4}`, `runtime.torch_npu_version=2.10.0`, `runtime.torch_npu_available=true`, `runtime.torch_npu_device_count=1`, and `runtime.ascend_env.ASCEND_RT_VISIBLE_DEVICES=0`. The artifact auditor passes for the current PR evidence with `full_checkpoint_forward_passed=false` and `multicard_passed=false`, so full 869 GB payload forward parity and multi-card SP/EP/FSDP2 parity are still not claimed in this report. A valid full-checkpoint artifact must set top-level `full_checkpoint_load_executed=true`, keep `failed=[]`, record runtime/tolerance evidence, and pass the auditor in `--require-full-checkpoint-forward` mode. A valid multi-card artifact must come from `run_minimax_m3_vl_multicard_parity.sh`, keep `multicard_parity_summary.json.passed=true`, include preflight/dummy/e2e logs, record `--runxfail`, prove `device_count>=min_devices` with `transformers>=5.12.0`, and pass the auditor in `--require-multicard` mode.
...
3. **SP/EP e2e alignment.** `tests/e2e/test_e2e_parallel.py` now uses a MiniMax-specific dummy VLM dataset with 1-D position ids and MiniMax image/video masks, and `scripts/multimodal/run_minimax_m3_vl_multicard_parity.sh` provides the target-machine launcher for FSDP2 asymmetric dummy-forward plus SP/EP/FSDP2 e2e alignment with `--runxfail`. The artifact auditor now has `--multicard-json` and `--require-multicard`, so the alignment case remains incomplete until the launcher produces `multicard_parity_summary.json` with `passed=true` on multi-card hardware and that summary passes strict audit.
