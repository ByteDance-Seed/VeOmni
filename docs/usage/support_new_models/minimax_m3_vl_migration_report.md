...
3. **SP/EP e2e alignment.** `tests/e2e/test_e2e_parallel.py` now uses a MiniMax-specific dummy VLM dataset with 1-D position ids and MiniMax image/video masks, and `scripts/multimodal/run_minimax_m3_vl_multicard_parity.sh` provides the target-machine launcher for FSDP2 asymmetric dummy-forward plus SP/EP/FSDP2 e2e alignment with `--runxfail`. The alignment case remains incomplete until the launcher produces `multicard_parity_summary.json` with `passed=true` on multi-card hardware.
