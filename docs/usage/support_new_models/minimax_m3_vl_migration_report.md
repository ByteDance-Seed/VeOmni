...
The script reads local public safetensors payloads or remote safetensors tensor byte ranges, applies `MiniMaxM3VLCheckpointTensorConverter` to real tensors, compares converted key/shape/dtype metadata against the VeOmni generated model state, and records value fingerprints for sampled payload evidence. The remote sample reads 3 real tensor payloads from Hugging Face `model-00003-of-00059.safetensors` via HTTP Range (`payload_bytes_read=1024`), maps sparse-attention indexer norms and the MoE correction bias into generated VeOmni state keys, and passes with `shape_mismatch_count=0`, `missing_model_key_count=0`, and only the expected `F32->BF16` runtime cast for the correction bias. With `--mode forward --confirm-full-load`, the same script can load the complete local checkpoint into the upstream transformers reference and the VeOmni generated model, then compare fixed-prompt logits, top-k ids, and greedy decode ids. Full 869 GB payload forward parity is still not claimed in this report.
