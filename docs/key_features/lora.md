...
LoRA support requires the `peft` library, which is bundled into the `gpu` /
`npu` extras. A standard install is sufficient — there is no separate `lora`
extra any more:

```shell
uv sync --extra gpu --group dev
# or for Ascend NPU x86:
