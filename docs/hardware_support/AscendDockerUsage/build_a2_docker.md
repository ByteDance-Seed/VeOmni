...
`Dockerfile.ascend_9.1.0_torch_npu2.10.0.post4_910b.x86` for x86 or
`Dockerfile.ascend_9.1.0_torch_npu2.10.0.post4_910b.arm` for ARM64. These Dockerfiles include
`torch-npu==2.10.0.post4`, `triton-ascend`, and `fla_npu`.
...
docker build \
  ...
  -t ascend-a2-env:v1 \
  -f docker/ascend/Dockerfile.ascend_9.1.0_torch_npu2.10.0.post4_910b.x86 \
  .
```
...
docker build \
  ...
  --build-arg no_proxy=localhost,127.0.0.1 \
  -t ascend-a2-env:v1 \
  -f docker/ascend/Dockerfile.ascend_9.1.0_torch_npu2.10.0.post4_910b.arm \
  .
```
...
- torch-npu 2.10.0.post4, triton-ascend, and fla_npu for GDN
...
The x86 image sets `UV_NO_SYNC=1` so normal `uv run` commands preserve the
image-specific torch-npu post4, triton-ascend, and fla_npu overlays. Running an
explicit `uv sync` reconciles the environment with `uv.lock` and may remove
those overlays; rebuild the image or reinstall the image-specific overlays afterward.
...
#### x86 Dockerfile (Dockerfile.ascend_9.1.0_torch_npu2.10.0.post4_910b.x86)
...
#### ARM64 Dockerfile (Dockerfile.ascend_9.1.0_torch_npu2.10.0.post4_910b.arm)
1. Sets up the Ubuntu 22.04 base with Ascend CANN
2. Configures system dependencies and development tools
