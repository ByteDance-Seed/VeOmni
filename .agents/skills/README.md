...
| `veomni-migrate-transformers-v5` | Add or refresh a model's patchgen path under `veomni/models_kernel/transformers/<model>/generated/` (text + MoE + Omni). Use when porting upstream changes or adding a new model to the v5 patchgen flow. |
| `veomni-new-op` | Adding a new optimized kernel/operator to `veomni/kernels/` |
