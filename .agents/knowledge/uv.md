...
│   └── transformers-stable  transformers==5.8.1 (default, in default-groups)
├── [tool.uv]
...
`transformers==5.8.1` is pinned by the `transformers-stable` dependency
...
patch release; pip users should `pip install transformers==5.8.1` manually.
...
```bash
# Initial setup (transformers==5.8.1 via the default dependency group)
...
6. **`transformers==5.8.1` is the only supported version** — pinned via the `transformers-stable` default dependency group. New code targets v5 APIs (FSDP2 + patchgen-generated modeling) only.
