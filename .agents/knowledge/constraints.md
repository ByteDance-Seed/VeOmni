...
7. **SP uses all-to-all head/sequence exchange, not all-gather**
   ...
   - Local parallel state: `BaseTrainer._setup()` calls `register_parallel_state("base")` **before** seed/determinism env vars (`NCCL_DETERMINISTIC`, etc.). Initialization caches by topology and registers under `name`; duplicate name → warn and return existing. Never store the ParallelState **object**: `VeOmniModelRuntime.parallel_state` is a by-name registry lookup on every access and `scoped()` wraps `use_parallel_state(self.runtime_name)`, so the registry stays authoritative. Only the *name* is held. `clear_parallel_state()` after `destroy_process_group()`.
