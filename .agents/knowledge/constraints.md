...
7f. **DDP must materialize and load meta-init weights itself, before the wrap**
   ...
   - `_materialize_and_load_weights()` also takes `weights_path` as a `Mapping[str, str]` — `{sub_module_name: snapshot_path}` over `model.named_children()`, a strict bijection — for V2 OmniModel, where each declared sub-module owns its own HF folder. The mapping branch refuses `is_peft_model=True`: cross-sub-module PEFT semantics are undefined.
