...
22. **NPU (Ascend) code paths require guards**
    ...
    - NPU kernels live in `veomni/kernels/_kernels/` (RMSNorm is `rms_norm/<variant>/npu.py`) and leftover `veomni/ops/kernels/rotary/npu.py` plus `veomni/ops/platform/npu/` — they must not be imported on GPU-only environments.
