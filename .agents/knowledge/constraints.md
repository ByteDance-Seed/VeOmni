...
7. **SP uses all-to-all head/sequence exchange, not all-gather**
   ...
   - Local parallel state: `init_parallel_state()` returns the built `ParallelState` (and caches it by topology). `set_parallel_state(ps)` / `use_parallel_state(ps)` (a contextmanager) swap the current global state, so any code inside `with use_parallel_state(ps):` resolves its SP/DP/CP/EP groups from `ps`'s mesh. Every trainer scopes its parallel state in exactly **two regions**: (1) the **build region** — one `with use_parallel_state(self.parallel_state):` around the whole build sequence in `__init__` (right after `_setup()`), covering meta-init, FSDP2/EP wrap + weight load, optimizer grouping, and the SP data pipeline; and (2) the **run region** — `forward_backward_step` wraps `model(...)` forward + loss-reduce + backward. This matters because `init_parallel_state()` only sets the module global on its *first* call, so once multiple modules each build under their own state, the explicit scope keeps each module on its own mesh (a no-op for the single-model case). Do NOT re-wrap individual build helpers — the `__init__`-region wrap already covers them. This holds for `TextTrainer`, `VLMTrainer`, `DiTTrainer`, and `TextDPOTrainer` (all compose `BaseTrainer` via `BaseTrainer.__new__` and run their own build sequence + `train_step`).
