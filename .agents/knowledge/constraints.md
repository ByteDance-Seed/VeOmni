...
4. **Transformers version: pinned to v5.8.1**
   - VeOmni installs `transformers==5.8.1` via the `transformers-stable`
   ...
   - `is_transformers_version_greater_or_equal_to()` from
     ...
     gates (for HF APIs newer than 5.8.1) — do **not** add new
     `>= 5.0.0` … `>= 5.8.1` branches.
   - Patchgen regeneration must be done with `transformers==5.8.1` installed.
