...
7. **SP uses all-to-all head/sequence exchange, not all-gather**
   ...
   - Local parallel state: `BaseTrainer.register_parallel_state(name)` (after `_setup`) calls `init_parallel_state(name=...)`, which caches by topology and registers under `name` (this round every trainer passes `"base"`). Duplicate name → warn and return existing. Trainers do not store the ParallelState object or name — use `use_parallel_state("base")` / `get_parallel_state_by_name("base")`. `clear_parallel_state()` after `destroy_process_group()`.
   - **Build scope**: `_setup()` → `register_parallel_state("base")` → one `with use_parallel_state("base"):` around the whole build sequence. Do NOT re-wrap individual build helpers.
   ...
   - See `docs/superpowers/specs/2026-07-15-local-parallel-state-registry-design.md`.
