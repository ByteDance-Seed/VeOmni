...
27. **List arguments on the command line are space-separated and unbracketed**
    - `_add_arguments_recursive()` in `veomni/arguments/parser.py` maps every `List[T]` field to `nargs="+"` with `action=_ListArgumentAction`, so `--train.optimizer.muon_head_split_modules [q_b_proj]` would otherwise parse to the literal `['[q_b_proj]']`. The action rejects a bracket-wrapped value group whose separators survived, rejects any comma-containing element, and converts the elements itself (argparse applies `type=` before an action can inspect the whole group). It is the single choke point for list args built from dataclasses; the hand-rolled parser in `tasks/data_sim.py` bypasses it but fails loudly because its element type is `int`.
