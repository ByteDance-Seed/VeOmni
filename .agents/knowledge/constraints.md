...
7b. **DDP must materialize and load meta-init weights itself, before the wrap**
   ...
   - `init_device == "cpu"` is refused on the DDP path. `device_ids` names an accelerator, so rank0's CPU replica cannot be wrapped, while every other rank builds empty and skips the load (`veomni/models/loader.py`). Refusing on every rank up front beats the alternative: rank0 dying inside DDP's constructor while the others read the whole snapshot from disk and then block in its first collective until the NCCL timeout.
