...
7e. **DDP + SP: bypass DDP multi-forward in Omni loop; AVG grads over `fsdp_group` in clip**
   - DDP wraps with `process_group=dp_group`. Enabling SP shrinks `dp` (`dp_size = world / sp_size`), so that allreduce is often a no-op while each rank still holds only a shard's param grads (Ulysses token shards, or Omni batch-dim shards). FSDP2 already syncs over `fsdp_group` (`dp_sp`); for DDP, `veomni_clip_grad_norm` / `veomni_omni_module_clip_grad_norm` AVG-allreduce `p.grad` over `fsdp_group` when `sp_size > 1` before clipping.
   - Looped SP (`run_sp_looped_endpoint`) issues ``sp_size`` forwards before one backward — keep the DDP wrapper, set `broadcast_buffers=False` under SP (buffer inplace / Embedding version errors), and call the **raw** module inside the loop so the reducer is not entered `sp_size` times (manual `fsdp_group` AVG still runs at clip). Leading-dummy batch layout may leave some ranks with zero *local* grads; that is fine once `fsdp_group` AVG has run.
