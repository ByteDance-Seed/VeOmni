...
7e. **DDP + SP: `broadcast_buffers=False`; AVG grads over `fsdp_group` in clip**
   ...
   - Set `broadcast_buffers=False` under SP: DDP's default in-place buffer `copy_` at each forward (including constant index buffers such as SigLIP `position_ids` that `nn.Embedding` saves for backward) can fail autograd version checks (PyTorch #22095 / #66504). Module weights still sync via the gradient allreduce. Non-SP DDP keeps the default (BatchNorm running stats).
