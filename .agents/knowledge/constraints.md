...
Core entry points:
- `veomni/distributed/parallel_state.py` — `init_parallel_state_from_accelerator()`, `ParallelState` dataclass
...
6. **Device mesh initialization (`init_parallel_state_from_accelerator()`)**
...
7. **SP uses all-to-all head/sequence exchange, not all-gather**
   ...
   - Process groups: `comm.py` has NO group globals and NO setters. Its getters (`get_ulysses_sequence_parallel_group`, `get_unified_sequence_parallel_group`, `get_context_parallel_group`, `get_data_parallel_group`) resolve from the *current* `ParallelState`'s device mesh (`get_parallel_state().{ulysses,sp,cp,dp}_group`) — exactly how `fsdp_group` already worked. See 7d. Meshless SP is unsupported — `ParallelState.__post_init__` raises if `sp_enabled and device_mesh is None`; always build via `init_parallel_state_from_accelerator` (production) or `_init_parallel_state` (tests only).
   - Local parallel state: `BaseTrainer.setup_distributed(args)` registers `"base"` **before** seed/determinism env vars (`NCCL_DETERMINISTIC`, etc.). Initialization caches by topology and registers under `name`; duplicate name → warn and return existing. Never store the ParallelState **object**: `VeOmniModelRuntime.parallel_state` is a by-name registry lookup on every access, and every scope is opened as `use_parallel_state(self.model_name)`, so the registry stays authoritative. Only the *name* is held. `clear_parallel_state()` after `destroy_process_group()`.
   - **Build scope**: `setup_distributed(args)` registers `"base"`, then `VeOmniModelRuntime.setup()` scopes the whole model build (meta-init, freeze, FSDP2/EP wrap + weight load, optimizer) to the mesh it owns. A trainer must NOT open a build scope of its own or resequence those steps from outside: a model whose build differs subclasses the runtime.
   - **Run scope (per-op, not whole `train_step`)**: each ambient-dependent op gets its own wrap with `"base"` — `model` forward, `postforward`/`mean_global_loss`, `loss.backward()`, and grad clipping (`VeOmniModelRuntime.clip_grad_norm()` enters its own scope). When an API takes explicit `group=`, pass `get_parallel_state_by_name("base").sp_group` and skip ambient.
...
7f. **DDP must materialize and load meta-init weights itself, before the wrap**
   ...
   - `weights_path` is `None`, one snapshot path, or a `{child_name: snapshot_path}` mapping keyed on `model.named_children()`. The mapping form is the SeedOmni `fsdp_scope='model'` path: `OmniModelRuntime` wraps the composed `OmniModel` once and loads each split-checkpoint subfolder into the matching child. Per-module wrap (`fsdp_scope='module'`, the default) still calls `build_parallelize_model()` once per `ModuleRuntime` with that module's `args.model_path`.
   - SeedOmni `fsdp_scope='model'` wraps the composed `OmniModel` once, and its wrap targets are each child's `_no_split_modules` **scoped as `{child}.{ClassName}`** (`_scoped_no_split_modules`, matched by `_is_fsdp_wrap_target`), replacing HF `post_init`'s flat class-name union. A bare class name is ambiguous across children: `Embedding` is a correct unit under the text encoder (its tied head gathers the weight through `EmbParallelMixin.emb_local_weight`) but the same class under the VQVAE shards the codebook, and `JanusVQVAEVectorQuantizer.forward` reads `self.embedding.weight` directly — no `__call__`, so no unshard hook, so `aten.add.Tensor` gets mixed Tensor/DTensor. General rule: an FSDP wrap boundary must sit at or above the module that reads a `.weight` directly, never strictly below it (this is why HF's own `JanusVQVAE._no_split_modules` lists the quantizer, not the embedding). Leftover params (aligner, final norm, anything not in a nested unit) unshard because training enters `OmniModel.forward()` via `nn.Module.__call__` — `OmniModelRuntime.forward` must call `self.model(batch)`, not run the graph outside the wrapped root.
...
8. **EP shards expert weights and exchanges tokens via all-to-all**
   ...
   - Device mesh: mesh init builds `[ep × ep_fsdp]` submesh; accessed via `ParallelState.extra_parallel_mesh("ep")`, `ep_group`, `ep_rank`.
...
26. **DCP full resume skips HF weight materialization only when DCP owns the model restore**
    - The decision is a property, not a caller argument: `VeOmniModelRuntime.skip_hf_weight_load` returns `should_skip_hf_weight_load(train.checkpoint.load_path, args.lora_config)`, i.e. `True` when `train.checkpoint.load_path` is set and the run is not LoRA/PEFT. A runtime whose DCP will *not* restore the model overrides the property to veto it (`DPOReferenceModelRuntime`, `ModuleRuntime`) — there is no `skip_hf_weight_load=` parameter on `build_parallelized_model` to pass a veto through. The value is forwarded through `build_parallelize_model` to `parallelize_model_fsdp2` / `parallelize_model_ddp`.
    - When it is `True`, the model is materialized without an HF weight read and its parameters must be restored by DCP in `ModelDcpCallback.on_train_begin`.
    ...
    - SeedOmni V2 decides this per `ModuleRuntime` (`ModuleRuntime.skip_hf_weight_load`, consulted by the inherited `build_parallelized_model`). A fully frozen module with a non-empty `state_dict()` gets NO checkpoint manager from `build_checkpoint`, so nothing restores its persistent model state; it must veto the skip and load its released HF weights even during resume. The property is read after `freeze_model`, so `has_trainable_parameters` already reflects freezing + LoRA.
