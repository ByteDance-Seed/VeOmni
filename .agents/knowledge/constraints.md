...
7. **SP uses all-to-all head/sequence exchange, not all-gather**
   ...
   - Process groups: `comm.py` has NO group globals. Its getters (`get_ulysses_sequence_parallel_group`, `get_unified_sequence_parallel_group`, `get_context_parallel_group`, `get_data_parallel_group`) resolve from the *current* `ParallelState`'s device mesh (`get_parallel_state().{ulysses,sp,cp,dp}_group`) — exactly how `fsdp_group` already worked. `set_ulysses_sequence_parallel_group(group)` survives ONLY as a unit-test injection seam (`_ULYSSES_SP_GROUP_OVERRIDE`, `None` in production); no production code calls it. Meshless SP is unsupported — `ParallelState.__post_init__` raises if `sp_enabled and device_mesh is None`; always build via `init_parallel_state_from_config`. See 7d.
   ...
   - **Run scope (per-op, not whole `train_step`)**: each ambient-dependent op gets its own wrap with this model's ParallelState name (`"base"` for a single-model trainer; `"policy"` / `"reference"` on `TextDPOTrainer`) — `model` forward, `postforward`/`mean_global_loss`, `loss.backward()`, and grad clipping (`VeOmniModelRuntime.clip_grad_norm()` enters its own scope). When an API takes explicit `group=`, pass `get_parallel_state_by_name(<that name>).sp_group` and skip ambient.
...
7f. **DDP must materialize and load meta-init weights itself, before the wrap**
   ...
   - `broadcast_model_weights_from_rank0` is honoured here under both FSDP2 and DDP. `rank0_load_and_broadcast_weights()` broadcasts over the default (world) group from global rank0 (`dist.broadcast(..., src=0)`, no `group=`), which is what a DDP replica wants. It defaults to `True`. A test that stubs the loader must make the stub materialize, exactly as a real one does.
...
26. **DCP full resume skips HF weight materialization only when DCP owns the model restore**
    - The decision is a property, not a caller argument: `VeOmniModelRuntime.skip_hf_weight_load` returns `should_skip_hf_weight_load(train.checkpoint.load_path, args.lora_config)`, i.e. `True` when `train.checkpoint.load_path` is set and the run is not LoRA/PEFT. A runtime whose DCP will *not* restore the model overrides the property to veto it (`DPOReferenceModelRuntime`, `ModuleRuntime`) — there is no `skip_hf_weight_load=` parameter on `_build_parallelized_model` to pass a veto through. The value is forwarded through `build_parallelize_model` to `parallelize_model_fsdp2` / `parallelize_model_ddp`.
    - When it is `True`, the model is materialized without an HF weight read and its parameters must be restored by DCP in `CheckpointCallback.on_train_begin`.
