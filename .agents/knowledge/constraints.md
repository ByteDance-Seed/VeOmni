...
7b. **DDP must materialize and load meta-init weights itself, before the wrap**
   ...
   - Only `init_device == "meta"` gets that pass. `cuda` and rank0 under `cpu` arrive with weights already loaded by the model builder (`empty_init=False` in `veomni/models/auto.py`), and re-loading would be wasted work at best.
   ...
   - `broadcast_model_weights_from_rank0` stays fsdp2-only — the arguments layer already warns that it is ignored elsewhere — so the DDP path reads on every rank. DDP's own constructor broadcast then makes rank0 authoritative regardless.
   - `init_device` is not the only route to meta: under `cpu`, every rank but 0 builds empty and skips the load (`veomni/models/loader.py`), and that config cannot work with DDP anyway because rank0's parameters stay on CPU while `device_ids` names an accelerator. `parallelize_model_ddp()` therefore re-checks `param.is_meta` after the load pass and raises a named error, rather than leaving the next reader to decode `Tensor.item()`.
   - ExtraParallel is refused on the DDP path. Only `parallelize_model_fsdp2()` applies the plan that shards expert weights, so DDP experts are whole; loading a sharded-expert config into them used to be prevented only by the meta crash, and silently loading full tensors instead would be worse than failing. `ep_sharded_stream_load` is rejected for the same reason instead of being ignored.
...
26. **DCP full resume skips HF weight materialization**
    - When `train.checkpoint.load_path` is set and the run is not LoRA/PEFT, `BaseTrainer` / omni train pass `should_skip_hf_weight_load=True` into `build_parallelize_model`, which forwards it to `parallelize_model_fsdp2` / `parallelize_model_ddp`. All three reject the former name, `skip_weights_load`, rather than swallowing it into `**kwargs`.
