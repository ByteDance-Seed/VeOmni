...
7-outer. **SP is UNIFORM across the outer trainer and every module (Arch B); the dataloader REPLICATES each DP shard across the SP group**
   ...
   - The outer trainer carries the SP size via the **top-level** `accelerator.ulysses_size` (CLI `--accelerator.ulysses_size`). NOTE: for `OmniArguments` the top-level `accelerator` is canonical — `__post_init__` does `self.train.accelerator = self.accelerator`, so a nested `--train.accelerator.ulysses_size` is silently overwritten (SP stays 1 with no error). Its dataloader is the standard `BaseTrainer` build-time sharded loader: it yields `dp_size = world / sp_size` **distinct** shards and **replicates** each shard across its SP group (no per-modality slicing in the collator — slicing happens inside each SP module). So every rank of an SP group holds the SAME sample.
   - **Uniform SP is hard-enforced:** `OmniTrainer._validate_uniform_sp()` raises at build time unless every module's `ulysses_size` equals the outer trainer's. A module at a different size would form a mismatched process group and slice against the wrong replication factor. Configs therefore set the outer `ulysses_size` and let modules inherit it (no per-module overrides).
...
7b. **Omni decode losses reduce over `dp_sp` (`fsdp_group`), NOT just `sp_group` — token-weighting must be global for SP transparency**
   ...
   - **Consequence — the collective spans every DP rank, so decode must run symmetrically on ALL `fsdp_group` ranks.** Any per-rank early-return before the reduce (e.g. vqvae's old `is_dummy` fast-path) will desync/hang when dummy-ness varies across DP. `vqvae.decode` therefore always routes through `_vq_loss`; an all-dummy span is handled by its `-100` labels (CE 0, clamped denom → 0.0, `generation_head` still runs as the FSDP anchor). This changes DP loss semantics for Omni training from mean-of-means to global token-weighted (bagel excluded, out of SP scope).

7c. **Per-module metric meter must stash PRE-slice (full-sample) seqlens in `pre_forward`, not measure the sliced forward `data`**
   - `MetricMeterMixin` no longer reads the module's forward `data` for token counts. Each metered module calls `metric_meter_set_seqlens(method, seqlens)` **inside `pre_forward`, before the SP slice**; the default `metric_meter_token_lengths` just drains that stash. Backbones (`janus_llama`, `qwen3`, `qwen3_moe`) build it from the full `position_ids`, `text_encoder` from the full `input_ids`, `janus_siglip`/`janus_vqvae` from the full image count, `bagel/siglip_navit` from `token_lens` (no SP → already full).
...
7e. **DDP buffers: broadcast persistent ones, ignore config-derived ones; AVG grads over `fsdp_group` in clip**
   - Under uniform SP a DDP-wrapped module (e.g. `janus_siglip`) still runs a single forward/backward (classic single-pass Ulysses, no per-sample loop). DDP wraps with `process_group=dp_group`; enabling SP shrinks `dp` (`dp_size = world / sp_size`), so that allreduce is often a no-op while each rank still holds only a shard's param grads. FSDP2 already syncs over `fsdp_group` (`dp_sp`); for DDP, `veomni_clip_grad_norm` / `veomni_omni_module_clip_grad_norm` AVG-allreduce `p.grad` over `fsdp_group` when `sp_size > 1` before clipping.
   - Keep DDP's default buffer broadcast, but list every `persistent=False` buffer in `_ddp_params_and_buffers_to_ignore` (`_non_persistent_buffer_names`). Buffers get no gradients, so the gradient allreduce never covers them and this broadcast is their ONLY sync path — hence splitting on `persistent` rather than disabling it wholesale. Config-derived buffers (SigLIP `position_ids`, rotary `inv_freq`) are derived state, not replicated state needing alignment — a static rope table is identical on every rank anyway, and dynamic-rope `inv_freq` is recomputed from *this* rank's sequence length (`modeling_rope_utils.py` re-registers it in forward), so broadcasting rank0's copy would actively corrupt the others. Ignoring them is therefore correct, not merely free. Meanwhile DDP's in-place `copy_` at the start of each forward corrupts any buffer that a live autograd graph saved for backward (PyTorch #22095 / #66504). That fires whenever a module is entered twice before one backward — which `call_graph_endpoint` does for every module owning more than one graph node, since `encode`/`decode` are trampolined through `wrapped(**kwargs)` so the DDP pre-forward hooks run per node. Persistent buffers (BatchNorm running stats) stay broadcast, so they no longer silently diverge per rank and get rank0's copy baked into the checkpoint. The ignore list is keyed by buffer NAME, which survives dynamic rope replacing the tensor object: DDP's `_assign_modules_buffers` re-collects `named_buffers()` and re-applies the name filter on every sync, precisely because modules reassign buffers (pytorch#63916).
   - FSDP2 has NO buffer sync at all (torch's whole `distributed/fsdp/` tree contains no `_sync_module_states` / `_broadcast_coalesced`; `fully_shard` only places buffers on the device). So mutable buffers drift across ranks under FSDP2 unconditionally — the DDP broadcast above has no FSDP2 counterpart to mirror.
   - Residual gap: a DDP module with BOTH persistent buffers that are saved for backward AND ≥2 graph nodes still raises the version-counter error — loudly, not silently. Use `SyncBatchNorm` for anything needing synchronized running stats: it all-reduces them inside forward, so it is immune to both the FSDP2 gap and the ≥2-node DDP case, and it is the semantically correct choice anyway (broadcast-from-rank0 discards the other ranks' statistics). Under SP its `process_group` must span dp+sp, since Ulysses splits the sequence and `dp_group` excludes SP peers. The repo currently has zero plain `nn.BatchNorm*` (generative decoders use GroupNorm, which has no buffers), so this is forward-looking, not a live bug.
...
26. **DCP full resume skips HF weight materialization only when DCP owns the model restore**
    - `BaseTrainer` combines the caller's permission with the generic resume check: `effective_skip = caller_skip and should_skip_hf_weight_load(load_path, lora_config)`. A caller veto (`False`) must short-circuit the generic helper; the default (`True`) preserves standalone trainer behavior.
    - When the effective value is `True`, the model is only `to_empty()`-materialized and its parameters must be restored by DCP in `CheckpointerCallback.on_train_begin`.
    - SeedOmni V2 decides this per `OmniModuleTrainer`. A fully frozen module with a non-empty `state_dict()` uses `_FrozenModuleNoOpCkptCallback`, so no DCP callback restores its persistent model state; it must veto the skip and load its released HF weights even during resume.
