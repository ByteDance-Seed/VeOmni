...
27. **List arguments accept space-separated values and the YAML flow sequence**
    - `_add_arguments_recursive()` in `veomni/arguments/parser.py` maps every `List[T]` field to `nargs="+"` with `action=_ListArgumentAction`, so `--train.optimizer.muon_head_split_modules [q_b_proj]` would otherwise parse to the literal `['[q_b_proj]']`. The action rejoins the value group and, when it is bracket-wrapped, hands it to `yaml.load(..., Loader=yaml.BaseLoader)`: real YAML rather than a comma split, so quoted elements and malformed sequences behave as they would in a config file, and `BaseLoader` leaves conversion to the field's element type. Anything YAML rejects or that is not a list of scalars stays literal, and literal values may not contain a comma. It converts the elements itself, because argparse applies `type=` before an action can inspect the whole group. It is the single choke point for list args built from dataclasses; the hand-rolled parser in `tasks/data_sim.py` bypasses it but fails loudly because its element type is `int`.
    - One list syntax across both surfaces is deliberate: CLI overrides beat YAML, so a config copy-paste used to silently defeat a correct config file. Mixing the separators is rejected in both directions — a comma outside brackets, and whitespace inside a bracketed element (`'[a b]'`, which YAML reads as one element). The remaining cost is that an element which is *only* a bracket group (`'[0-9]'`) loses its brackets unless it is quoted inside the sequence (`'["[0-9]"]'`); a bracket elsewhere in the group (`data/part-[0-9]`, `'[a]' '[b]'`) is what keeps values literal.
    - Rejecting brackets never protected against the other hazard: unquoted, `[q_b_proj]` is a shell glob character class, and once the shell rewrites it the parser sees no brackets at all. That is why consumers must fail loudly on their own (next bullet), and why docs tell users to quote the bracketed form.
