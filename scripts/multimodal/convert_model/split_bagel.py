"""Split BAGEL-7B-MoT into SeedOmni V2 module checkpoints.

This splitter follows the module-first SeedOmni workflow: instantiate the
registered Bagel module classes, load the upstream weights into those module
objects, then save each module with ``save_pretrained()``.

The released BAGEL checkpoint is a two-file layout:

* ``ema.safetensors``: language model, SigLIP NaViT, and flow connector weights
* ``ae.safetensors``: FLUX/BAGEL autoencoder weights
"""
...
def _prepare_output_dir(output_dir: Path, force: bool) -> None:
    if output_dir.exists() and any(output_dir.iterdir()):
        if not force:
            raise FileExistsError(f"{output_dir} already exists and is not empty. Pass --force to overwrite it.")
...
def split_bagel(model_path: str, output_dir: str, *, force: bool = False, max_latent_size: int = 64) -> None:
    ...
    siglip_state = _load_prefixed_safetensors(
        ema_path,
        lambda key: key.removeprefix("vit_model.") if key.startswith("vit_model.") else None,
        consumed_ema_keys,
    )
    ...
    flow_prefixes = ("connector.", "vae2llm.", "llm2vae.", "time_embedder.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Split BAGEL-7B-MoT into SeedOmni V2 modules")
