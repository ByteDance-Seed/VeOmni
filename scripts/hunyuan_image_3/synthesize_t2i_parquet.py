...
"""CLI to write a synthetic T2I parquet for smoke / overfit runs.

Examples:

    # Single 1024px bucket, single prompt length.
    python3 scripts/hunyuan_image_3/synthesize_t2i_parquet.py \
        --out /tmp/hy3_stage_A.parquet \
        --n_rows 128 --image_sizes 1024,1024 --prompt_lengths 128

    # Multi-resolution + multi-length prompts.
    python3 scripts/hunyuan_image_3/synthesize_t2i_parquet.py \
        --out /tmp/hy3_stage_B.parquet \
        --n_rows 192 --image_sizes 512,512:768,768:1024,1024 \
        --prompt_lengths 32,128,256

    # Overfit set: identical rows for memorization.
    python3 scripts/hunyuan_image_3/synthesize_t2i_parquet.py \
        --out /tmp/hy3_overfit.parquet \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Write a synthetic T2I parquet for smoke runs.")
