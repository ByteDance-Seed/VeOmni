...
if ! python -c "import pyarrow" 2>/dev/null; then
  ...
  echo "[debug_omni] Install: uv pip install pyarrow" >&2
  exit 1
fi
...
DEBUG_ROOT="${REPO_ROOT}/outputs/debug_omni"
DEBUG_DATA="${DEBUG_ROOT}/data"
mkdir -p "${DEBUG_DATA}"
...
torchrun \
  ...
  --train.checkpoint.output_dir "${DEBUG_ROOT}/${DATASET_MODE}" \
