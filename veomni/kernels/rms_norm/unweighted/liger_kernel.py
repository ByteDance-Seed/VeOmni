...
def forward(x: Tensor, *, eps: float) -> tuple[Tensor, SavedState]:
    """Liger fused RMSNorm without an affine weight (llama casting).

    Empty ``x`` falls back to the eager pair. A non-``None`` weight grad
    from Liger is treated as a contract violation.
    """
    if x.numel() == 0:
...
def backward(grad_output: Tensor, saved: SavedState) -> tuple[Tensor]:
    """Return ``(grad_x,)``. Empty inputs reuse the eager backward."""
    meta = saved.metadata
