...
#  Generated: 2026-03-04T09:35:43.633421
#
    ...
    **Cache strategy**: The Indexer manages its own key cache (`_cached_keys`) separately
    ...
    def forward(
        ...
        The scoring logic computes:
            index_score[b,s,t] = ¦²_h (weight[b,s,h] ¡¤ softmax_scale ¡¤ q[b,s,h,:] ¡¤ k[b,t,:])
        ...
        # q¡¤k^T per head: [B, S, H, D] @ [B, T, D]^T ¡ú [B, S, H, T]
        scores = torch.einsum("bshd,btd->bsht", q.float(), k_cached.float()) * self.softmax_scale

        # Weight per head and sum across heads ¡ú [B, S, T]
...
class GlmMoeDsaAttention(nn.Module):
    ...
    This follows the same architecture as DeepSeek V3.2's MLA:
      - Query: x ¡ú q_a_proj ¡ú RMSNorm ¡ú q_b_proj ¡ú split(q_nope, q_pe) ¡ú RoPE(q_pe)
      - KV:    x ¡ú kv_a_proj ¡ú split(kv_compressed, k_pe) ¡ú RMSNorm(kv_compressed) ¡ú kv_b_proj
                                                           ¡ú RoPE(k_pe)
    ...
    **FP8 compatibility**: all weight accesses use standard nn.Linear forward calls (never
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Split nope/rope, apply RoPE, recombine ¡ª layout: [B, H, S, D]
