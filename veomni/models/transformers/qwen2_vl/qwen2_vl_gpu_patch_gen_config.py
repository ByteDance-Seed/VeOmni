...
Regen command:
patchgen veomni.models.transformers.qwen2_vl.qwen2_vl_gpu_patch_gen_config -o veomni/models/transformers/qwen2_vl/generated --diff
...
@config.override_method(
    "Qwen2VLRMSNorm.__init__",
    description="Construct a local rms_norm VeomniOp",
)
def qwen2_vl_rmsnorm_init_patched(self, hidden_size, eps: float = 1e-6) -> None:
...
@config.override_method(
    "Qwen2VLRMSNorm.forward",
    description="Always call the local rms_norm VeomniOp",
)
def qwen2_vl_rmsnorm_forward_patched(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
def qwen2_vl_mlp_init_patched(self, config):
    nn.Module.__init__(self)
    self.config = config
...
# ================================================================
@config.override_method(
    "Qwen2VLForConditionalGeneration.get_metadata_collate_func",
    description="Expose CPU-side ViT multimodal-metadata derivation to the VeOmni collator",
)
def qwen2vl_get_metadata_collate_func_patched(self):
...
@config.override_method(
    "Qwen2VLForConditionalGeneration.__init__",
    description="Bind ForCausalLMLoss to a local cross_entropy_loss VeomniOp",
)
def qwen2vl_for_conditional_generation_init_patched(self, config):
...
@config.modify_init("Qwen2VLAttention", description="Bind instance-local rope and attention VeomniOps")
