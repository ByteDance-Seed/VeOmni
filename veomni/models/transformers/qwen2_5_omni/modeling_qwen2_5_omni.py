...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(Qwen2_5OmniPreTrainedModel):
    ...
    def get_rope_index(
        ...
        audio_token_id = self.audio_token_index
...
class Qwen2_5OmniAudioFlashAttention2(Qwen2_5OmniAudioAttention):
    ...
    def forward(
        ...
        attn_output = attn_output.reshape(seq_length, all_dim).contiguous()
        attn_output = self.out_proj(attn_output)
...
class Qwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        # sp patch: use all-to-all to get full sequence of input_features
        ...
        # sp patch: use all-to-all to reshape back to sp input_features
        if get_parallel_state().sp_enabled:
        ...
        # sp patch: use all-to-all to gather all sequence
        if get_parallel_state().sp_enabled:
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        if feature_attention_mask is not None:  # for inference
            ...
            if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
                if input_features is not None:
                    ...
                    audio_outputs = self.audio_tower(
                        input_features, feature_lens=audio_feature_lengths, aftercnn_lens=audio_feat_lengths
                    )
                    audio_features = audio_outputs.last_hidden_state
                    if audio_features.shape[0] != sum(audio_output_lengths.tolist()):
                        raise ValueError("length of audio_features should match audio_output_lengths")
                    assert audio_features.shape[0] == audio_mask.sum(), (
                        "length of audio_features should match audio_mask"
                    )
                    audio_features = audio_features.to(inputs_embeds)

                    if get_parallel_state().sp_enabled:
                        # audio_features gathered in audio_tower
                        feature_dim = audio_features.shape[-1] // get_parallel_state().sp_size
                        sp_rank = get_parallel_state().sp_rank
                        audio_features = audio_features[:, sp_rank * feature_dim : (sp_rank + 1) * feature_dim]
                ...
                if pixel_values is not None:
                    ...
                    image_embeds = image_embeds.to(inputs_embeds)
...
class Qwen2_5OmniTalkerModel(Qwen2_5OmniPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if "flash_attention_2" in self.config._attn_implementation:
            if attention_mask is not None and past_key_values is not None:
                is_padding_right = attention_mask[:, -1].sum().item() != input_tensor.size()[0]
                if is_padding_right:
                    raise ValueError(
