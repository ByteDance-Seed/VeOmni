...
def register_qwen3_moe_modeling(architecture: str):
    if not is_transformers_version_greater_or_equal_to("5.0.0"):
        raise RuntimeError(
            "qwen3_moe is now v5-only in VeOmni. The legacy v4 monkey-patch path "
            "(`modeling_qwen3_moe.py` + `checkpoint_tensor_converter_v4.py`) was "
            "retired together with the broader transformers v4 wind-down. Upgrade "
            "to transformers >= 5.0.0 (e.g. install with the `transformers5-exp` "
            "extra) to keep using qwen3_moe."
        )
