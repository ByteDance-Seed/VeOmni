...
# 7. exactly-once vision tower execution when SP is disabled: one merged
#    image+video ViT call (or one single-modality call, or one dummy)
#    per rank per step instead of the two-slot real-or-dummy layout
# ================================================================
@config.override_method(
...
# ================================================================
# Patch: Qwen3VLForConditionalGeneration.get_pre_sp_collate_func (new)
# Expose the pre-SP pixel-stream merge (image+video -> pixel_values_merged)
# to VeOmni's collator so the vision tower runs exactly once per rank under
# SP. See Patch.7 in Qwen3VLModel.forward.
# ================================================================
@config.override_method(
    "Qwen3VLForConditionalGeneration.get_pre_sp_collate_func",
    description="Expose the pre-SP pixel-stream merge hook to the VeOmni collator",
)
def qwen3_vl_get_pre_sp_collate_func_patched(self):
