...
"""Frozen HunyuanImage 3 VAE encoder.

Adapted from Tencent's ``autoencoder_kl_3d.py`` at revision
``6e9113a692a27a0751d82aba3b2015a876646c03``. The container keeps the official
``vae.encoder.*`` state-dict hierarchy so checkpoint keys map identically.

Training only ever runs the encoder, and only to produce a latent posterior for
``prepare_reference_flow_batch``. Decoding is a separate future capability --
when it lands it adds a ``decoder`` submodule under the same prefix.
"""
...
class Encoder(nn.Module):
    ...
    def forward(self, tensor: Tensor) -> Tensor:
        # No activation checkpointing: the encoder is frozen and always runs
        # under ``torch.no_grad`` (see HunyuanImage3VAE.encode), so there is no
        # backward graph to trade compute against.
        ...
        for level in range(len(self.block_out_channels)):
            ...
            if hasattr(self.down[level], "downsample"):
                hidden = self.down[level].downsample(hidden)
...
class LatentPosterior:
    """The diagonal-Gaussian parameters the flow objective consumes.

    Training never samples from the posterior here -- ``prepare_reference_flow_batch``
    takes ``mean`` and ``logvar`` and reparameterises under the flow RNG, so a
    given step is reproducible.
    """
    ...
    @classmethod
    def from_encoder_output(cls, parameters: Tensor) -> "LatentPosterior":
        mean, logvar = torch.chunk(parameters, 2, dim=1)
        return cls(mean=mean, logvar=torch.clamp(logvar, -30.0, 20.0))


class HunyuanImage3VAE(nn.Module):
    """Frozen encoder container preserving the official ``vae.encoder.*`` prefix."""
    ...
    @torch.no_grad()
    def encode(self, pixel_values: Tensor) -> LatentPosterior:
        ...
        return LatentPosterior.from_encoder_output(self.encoder(pixel_values))


__all__ = ["HunyuanImage3VAE", "LatentPosterior"]
