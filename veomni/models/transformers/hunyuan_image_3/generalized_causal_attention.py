...
indices consumed here (see ``sequence_layout.compile_single_gen_t2i_packed``).
This dispatcher stays model-local and reuses the configured low-level
Flash-Attention backend rather than registering a new global attention op.

:func:`build_packed_gca_dense_mask` is the correctness oracle for the
decomposition: it materialises the same edge mask from the *same* packed
metadata, so an equivalence test needs no second compiler or model forward.
"""

import torch


# The compiled-metadata keys the model forward hands to this dispatcher, declared
# next to the code that reads them. The model slices ``gca_metadata`` out of the
# full compiled metadata with the wide tuple; the tensor subset is split out
# because only those entries need a device check.
...
def gca_varlen_attention_forward(
    ...
    *,
    ...
    prefix_output = attention_interface(
        ...
        scaling=scaling,
        is_causal=True,
        cu_seq_lens_q=gca_metadata["cu_seqlens_prefix"],
        cu_seq_lens_k=gca_metadata["cu_seqlens_prefix"],
...
def build_packed_gca_dense_mask(gca_metadata, *, device):
    """Materialise the ``[1, T, T]`` boolean edge mask the two calls encode.

    Correctness oracle, not a production path: O(T^2), built from the same packed
    metadata the fast path consumes, and block-diagonal *by construction* (no
    cross-sample edge can exist).

    Per sample the allowed set is ``tril`` — the causal prefix — with the image
    suffix square opened: every payload/``<eoi>`` token sees every other token in
    its own sample, in both directions.
    """
    cu_full = gca_metadata["cu_seqlens_k_full"].tolist()
    cu_prefix = gca_metadata["cu_seqlens_prefix"].tolist()
    total_length = gca_metadata["sequence_length"]

    mask = torch.zeros((1, total_length, total_length), dtype=torch.bool, device=device)
    for sample_index in range(len(cu_full) - 1):
        start, stop = cu_full[sample_index], cu_full[sample_index + 1]
        prefix_length = cu_prefix[sample_index + 1] - cu_prefix[sample_index]
        block = torch.ones((stop - start, stop - start), dtype=torch.bool, device=device).tril()
        block[prefix_length:, prefix_length:] = True
        mask[0, start:stop, start:stop] = block
    return mask
