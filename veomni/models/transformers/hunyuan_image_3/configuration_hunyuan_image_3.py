...
class HunyuanImage3Config(HunYuanMoEV1Config):
    ...
    # The shipped checkpoint carries no ``preprocessor_config.json`` and no
    # ``AutoProcessor`` entry, so the loader falls back to reading this
    # attribute and looking the name up in ``MODEL_PROCESSOR_REGISTRY``.
    processor_class = "HunyuanImage3Processor"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Unlisted components take their HunyuanImage3ComponentPolicy default.
        component_policy_values = kwargs.pop("component_policy", None) or {}
        self.component_policy = HunyuanImage3ComponentPolicy.from_dict(component_policy_values).as_dict()

        # Flow-matching objective. Static for a run, so it lives here rather than
        # riding on every micro-batch; normalize_flow_config fills the reference
        # defaults and rejects unsupported combinations at build time.
        self.flow = normalize_flow_config(kwargs.pop("flow", None))
