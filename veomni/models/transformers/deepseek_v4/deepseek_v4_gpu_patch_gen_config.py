...
config.add_post_import_block(
    ...
    def _deepseek_v4_build_flashmla_indices(
        ...
        *,
        ...
        query_positions = torch.arange(seq_len, device=top_k_indices.device, dtype=torch.int32).view(1, seq_len, 1)
        offsets = torch.arange(window - 1, -1, -1, device=top_k_indices.device, dtype=torch.int32).view(1, 1, window)
...
def deepseek_v4_indexer_forward_patched(
    ...
    if indexer_backend == "cudnn":
        from veomni.ops.kernels.deepseek_sparse_attention.common import indexer_scores
...
def deepseek_v4_attention_forward_patched(
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    if attention_backend in ("flashmla_cudnn", "fa4_cudnn") and self.layer_type == "compressed_sparse_attention":
        ...
        if unsupported_reasons:
            raise ValueError(
                f"dsa_attention_backend={attention_backend!r} is not supported: " + "; ".join(unsupported_reasons)
            )
        ...
        q_nope = F.pad(q_bshd[..., :-rope_dim], (0, rope_dim))
        ...
        attn_output = sparse_mla_attention_with_cudnn_backward(
            attention_backend,
            q_pe.contiguous(),
            k_pe.contiguous(),
            kv_bshd.contiguous(),
