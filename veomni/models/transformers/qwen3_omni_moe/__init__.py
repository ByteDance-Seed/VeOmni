...
def register_qwen3_omni_moe_modeling(architecture: str):
    if not is_transformers_version_greater_or_equal_to("5.2.0"):
        raise RuntimeError(
            "qwen3_omni_moe is now v5-only in VeOmni and requires transformers >= 5.2.0. "
            "The legacy v4 monkey-patch path (`modeling_qwen3_omni_moe.py` + "
            "`checkpoint_tensor_converter_v4.py`) was retired together with the broader "
            "transformers v4 wind-down. Install with the `transformers5-exp` extra or "
            "pin transformers>=5.2.0 to keep using qwen3_omni_moe."
        )
...
def register_qwen3_omni_moe_processor():
    ...
    # `if audio is not None` checks. These are data-format patches.
