    ...
    def forward(
        ...
        * ``"outputs"`` : ``{node_name: full output dict}`` as returned by
        ...
        for node_name in self.training_graph.execution_order:
            ...
            #
            # Scope the executor to its module's ParallelState so the free
            # function ``get_parallel_state()`` (read inside pre/forward/post —
            # e.g. the vocab-parallel ``emb`` embedding all-reduce, MoE EP groups)
            # resolves to this module's mesh. No-op when no per-module states are
            # registered (the print-flow tests).
