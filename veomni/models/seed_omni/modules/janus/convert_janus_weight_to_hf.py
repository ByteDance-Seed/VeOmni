...
def _load_sharded_state_dict(input_path: str, index_path: str, loader) -> dict:
    ...
    for shard_file in unique_shard_files:
        ...
        state_dict.update(loader(shard_path))
...
def load_model_state_dict(input_path: str) -> dict:
    ...
    Prefers safetensors (``model.safetensors`` / ``model.safetensors.index.json``),
    falling back to legacy PyTorch checkpoints when needed.
    """
    safetensors_index_path = os.path.join(input_path, "model.safetensors.index.json")
    safetensors_single_path = os.path.join(input_path, "model.safetensors")
    bin_index_path = os.path.join(input_path, "pytorch_model.bin.index.json")
    bin_single_path = os.path.join(input_path, "pytorch_model.bin")

    if os.path.exists(safetensors_index_path):
        from safetensors.torch import load_file

        print("Loading sharded safetensors model...")
        return _load_sharded_state_dict(
            input_path,
            safetensors_index_path,
            lambda shard_path: load_file(shard_path, device="cpu"),
        )

    if os.path.exists(safetensors_single_path):
        from safetensors.torch import load_file

        print("Loading single safetensors model...")
        return load_file(safetensors_single_path, device="cpu")
    ...
    if os.path.exists(bin_index_path):
        ...
        return _load_sharded_state_dict(
            input_path,
            bin_index_path,
            lambda shard_path: torch.load(shard_path, map_location="cpu"),
        )

    if os.path.exists(bin_single_path):
        print("Loading single PyTorch model...")
        return torch.load(bin_single_path, map_location="cpu")
