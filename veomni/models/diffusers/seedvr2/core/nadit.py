# Source: https://github.com/ByteDance-Seed/SeedVR/blob/e4de8c24441a67e1b7df56abea10645059bb1185/models/dit_v2/nadit.py
...
from veomni.models.diffusers.seedvr2.core.runtime import slice_inputs
...
# Non-reentrant checkpointing supports keyword arguments and the per-forward cache.
def gradient_checkpointing(module: Union[Callable, nn.Module], *args, enabled: bool, **kwargs):
    if enabled:
        kwargs.pop("cache", None)

        def run(*args, **kwargs):
            return module(*args, cache=Cache(), **kwargs)

        return checkpoint(run, *args, use_reentrant=False, **kwargs)
...
class NaDiT(nn.Module):
    ...
    def forward(
        ...
        if self.vid_out_norm:
            ...
            # Upstream reuses the attention branch's cached modulation here.
            # Select that slice explicitly so checkpoint recomputation does not
            # depend on a block populating the outer forward's mutable cache.
            output_emb = emb.reshape(emb.shape[0], -1, 2, 3)[:, :, 0, :].flatten(1)
            vid = self.vid_out_ada(
                ...
                cache=cache.namespace("output_modulation"),
                branch_tag="vid",
            )
