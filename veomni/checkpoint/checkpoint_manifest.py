...
"""Versioned checkpoint manifest guarding same-topology DCP resume.

A DCP checkpoint records nothing about the parallel mesh or the training semantics it
was produced with: resuming under a different mesh makes the load collective reshard
incorrectly or deadlock, and resuming under a different component/flow policy silently
changes the training. Saves write ``checkpoint_manifest.json`` beside the DCP files;
loads validate it on every rank *before* entering the DCP collective, so an
incompatible resume fails fast and identically everywhere instead of hanging.

Manifest version, model_type, mesh and every caller-supplied ``extra_hashes`` entry are
HARD gates (raise); ``soft_fields`` only warn. A missing manifest (checkpoint written
before this feature) is accepted with a warning. The manifest is model-agnostic --
models inject their own identity through ``extra_hashes``.
"""
