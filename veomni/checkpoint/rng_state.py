...
Checkpointing only the torch CPU generator lets Python's ``random``, NumPy and the CUDA
generators diverge across a resume; these helpers capture and restore all four so a
resumed run reproduces the continuous run's sampling (data shuffling, dropout,
flow/diffusion noise on non-derived paths, ...).

Every field is optional on restore, so a snapshot taken without CUDA or NumPy still
loads.
"""
