...
def strip_patch_decorators(source: str) -> str:
    """Remove patchgen decorator lines from source code, handling multi-line decorators.

    Strips any decorator that starts with ``@`` and contains ``config.`` or a known
    patchgen keyword (``replace_class``, ``replace_function``, ``override_method``).
    Multi-line decorators (spanning across parentheses) are fully removed.
    """
    keywords = ("replace_class", "replace_function", "override_method", "config.")
    ...
    for line in source_lines:
        stripped = line.strip()
        if not in_patch_decorator and stripped.startswith("@") and any(kw in stripped for kw in keywords):
