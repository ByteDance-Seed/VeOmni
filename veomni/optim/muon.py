...
def _root_mesh(mesh: Any) -> Any:
    """Return ``mesh``'s root mesh, or ``mesh`` itself when it is not a slice."""
    from torch.distributed.device_mesh import _mesh_resources

    root = _mesh_resources.get_root_mesh(mesh)
    return mesh if root is None else root
...
def _fsdp_all2all_submesh(p: DTensor) -> Optional[Any]:
    ...
    sharded dim out of the root mesh and let each replica reassemble the same
    rows on its own. That is the work split ``full_tensor()`` already
    performed, minus the redundant gather.

    Returns ``None`` for layouts the path cannot express (``Shard(d>0)``,
    several shard dims, unnamed multi-dim meshes), which then fall back to the
    generic ``full_tensor()`` path.
    """
    ...
    if not dim_names:
        return None
    return _root_mesh(mesh)[dim_names[mesh_dim]]


def _fsdp_all2all_fast_path_eligible(p: DTensor) -> bool:
    """True when ``p`` can take the owner-based all-to-all path.

    The path supports empty tail-rank shards when the first dimension is
    smaller than the mesh.
    """
    return _fsdp_all2all_submesh(p) is not None
