...
2. Diffusion / reference-training outputs (e.g. HunyuanImage 3) that
   don't fit the ``*WithLogProbs`` pattern but live here so GPU and NPU
...
class HunyuanImage3ReferenceOutput(ModelOutput):
    ...
    Declared here rather than in the patchgen config because a patchgen config
    body is inspected without importing PyTorch, so a ``ModelOutput`` subclass
    cannot live there. ``loss`` is a dict so image and auxiliary losses can be
    logged separately.
    """
