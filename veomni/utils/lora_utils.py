...
# fsdp1 meta device load lora weights will convert shard_states to peft namespace
def load_peft_shard_states(model: torch.nn.Module, adapter_path: str, shard_states: dict):
    ...
    for fqn, module in model.named_modules():
        ...
        if hasattr(module, "base_layer"):  # using lora peft will insert "base_layer"
            ...
            # Delegates to PEFT's reset_lora_parameters so any future changes to
            # PEFT's init logic are inherited automatically.
            #
            ...
            for adapter_name, lora_a_linear in getattr(module, "lora_A", {}).items():
                ...
                # Both keys already present (e.g. resume from checkpoint); nothing to init.
                if a_key in shard_states and b_key in shard_states:
                    continue
                if dist.get_rank() == 0:
                    if a_key not in state_dict:
                        logger.info_rank0(f"init lora weights for {a_key} and {b_key}")
                        a_w = lora_a_linear.weight
                        b_w = lora_b_linear.weight
                        dtype = torch.float32 if (a_w.is_meta or b_w.is_meta) else a_w.dtype
                        # Temporarily materialise on CPU so reset_lora_parameters can write to
                        # them. We then store the results in shard_states so create_and_sync_state
                        # broadcasts from rank 0. Other ranks store 0 (broadcast src = rank 0).
                        lora_a_linear.weight = nn.Parameter(torch.empty(a_w.shape, dtype=dtype))
                        lora_b_linear.weight = nn.Parameter(torch.empty(b_w.shape, dtype=dtype))
                        module.reset_lora_parameters(adapter_name, init_lora_weights=True)
                        shard_states[a_key] = lora_a_linear.weight.data.clone()
                        shard_states[b_key] = lora_b_linear.weight.data.clone()
                    else:
                        # load from state_dict
                        shard_states[a_key] = state_dict[a_key]
                        shard_states[b_key] = state_dict[b_key]
                else:
