...
_TTX_WARMUP_MARKERS: list[dict[str, Any]] = []
_TTX_WARMUP_MODE_ENV = "VEOMNI_TTX_WARMUP_MODE"
_TTX_WARMUP_MODES = frozenset({"direct_analytical", "legacy"})
_TTX_WARMUP_DEFAULT_MODE = "direct_analytical"
_TTX_WARMUP_DUMMY_TOKENS = 128


def _resolve_ttx_warmup_mode() -> str:
    raw = os.environ.get(_TTX_WARMUP_MODE_ENV, _TTX_WARMUP_DEFAULT_MODE)
    mode = str(raw).strip() or _TTX_WARMUP_DEFAULT_MODE
    if mode not in _TTX_WARMUP_MODES:
        raise RuntimeError(f"unknown TTX warmup mode {mode!r}; expected one of {sorted(_TTX_WARMUP_MODES)}")
    return mode


def _ttx_warmup_signature(key: Tensor, value: Tensor, g: Tensor, beta: Tensor) -> tuple:
    return (
        key.device.type,
        key.device.index,
        int(key.shape[2]),
...
def _ttx_warmup_signature_hash(signature: tuple) -> str:
    payload = "|".join(str(item) for item in signature)
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()[:16]


def _emit_ttx_warmup_marker(*, phase: str, mode: str, signature: tuple, origin: str = "runtime", **extra: Any) -> None:
    row = {
        "phase": phase,
        "mode": mode,
        "origin": origin,
        "sig_hash": _ttx_warmup_signature_hash(signature),
        "signature": [str(item) for item in signature],
        "rank": int(os.environ.get("RANK", os.environ.get("LOCAL_RANK", "-1"))),
    }
    row.update(extra)
    _TTX_WARMUP_MARKERS.append(row)
    print("AI4SE_TTX_WARMUP " + json.dumps(row, sort_keys=True, default=str), flush=True)


def _ttx_warmup_dummy_operands(
    key: Tensor,
    value: Tensor,
    g: Tensor,
    beta: Tensor,
    *,
    ...
    token_count = _TTX_WARMUP_DUMMY_TOKENS
    num_heads = int(key.shape[2])
    key_dim = int(key.shape[3])
    value_dim = int(value.shape[3])
    warm_key = torch.zeros(
        1, token_count, num_heads, key_dim, device=key.device, dtype=key.dtype, requires_grad=requires_grad
    )
    warm_value = torch.zeros(
        1, token_count, num_heads, value_dim, device=value.device, dtype=value.dtype, requires_grad=requires_grad
    )
    warm_g = torch.zeros(1, token_count, num_heads, device=g.device, dtype=g.dtype, requires_grad=requires_grad)
    warm_beta = torch.zeros(
        1, token_count, num_heads, device=beta.device, dtype=beta.dtype, requires_grad=requires_grad
    )
    ...
    return warm_key, warm_value, warm_g, warm_beta, warm_cu


def _warmup_direct_analytical(
    key: Tensor,
    value: Tensor,
    g: Tensor,
    beta: Tensor,
    *,
    ...
    """Compile TTX fwd/bwd by calling the kernels directly. No autograd.grad."""

    warm_key, warm_value, warm_g, warm_beta, warm_cu = _ttx_warmup_dummy_operands(
        key, value, g, beta, requires_grad=False
    )
    key_op, value_op, g_op, beta_op, _key_rstd, _normalized_key_fp32 = _prepare_ttx_forward_operands(
        warm_key,
        warm_value,
        warm_g,
        warm_beta,
    ...
    cu_pts = [int(point) for point in warm_cu.detach().cpu().tolist()]
    _emit_ttx_warmup_marker(phase="FWD_ENTER", mode=mode, signature=signature, origin=origin)
    hm = _ttx_local_affine_summary_fwd(
        key_op,
        value_op,
        g_op,
    ...
    _emit_ttx_warmup_marker(phase="FWD_EXIT", mode=mode, signature=signature, origin=origin)
    from veomni.ops.kernels.gdn_kcp_affine_ttx_bwd import ttx_local_affine_analytical_bwd

    _emit_ttx_warmup_marker(phase="BWD_ENTER", mode=mode, signature=signature, origin=origin)
    ttx_local_affine_analytical_bwd(
        key_op,
        value_op,
        g_op,
        beta_op,
    ...
    _emit_ttx_warmup_marker(phase="BWD_EXIT", mode=mode, signature=signature, origin=origin)
...
def _warmup_legacy(
    ...
    *,
    ...
    """Retained nested-autograd warmup. Do not use for the grouped-leaf experiment."""

    with torch.inference_mode(False), torch.enable_grad():
        warm_key, warm_value, warm_g, warm_beta, warm_cu = _ttx_warmup_dummy_operands(
            key, value, g, beta, requires_grad=True
        )
        _emit_ttx_warmup_marker(phase="FWD_ENTER", mode=mode, signature=signature, origin=origin)
...
def warmup_ttx_bc8_m1_forward_backward(
    ...
    *,
    ...
    Default mode ``direct_analytical`` calls the TTX forward kernel and
    ``ttx_local_affine_analytical_bwd`` directly so model forward never nests
    ``torch.autograd.grad``.  ``legacy`` keeps the previous autograd path.
    Unknown modes fail closed.  The cache is written only after both sides and
    the NPU sync succeed.

    ``origin`` distinguishes build-stage precompile (``build_pre_fsdp``) from
    the runtime readiness warmup that must then be ``CACHE_HIT`` only.
    """

    validate_ttx_bc8_m1_inputs(key, value, g, beta, cu_seqlens=None)
    signature = _ttx_warmup_signature(key, value, g, beta)
    mode = _resolve_ttx_warmup_mode()
    if signature in _TTX_FORWARD_BACKWARD_WARMUP_CACHE:
        _emit_ttx_warmup_marker(phase="CACHE_HIT", mode=mode, signature=signature, origin=origin)
        return
    if mode == "direct_analytical":
        _warmup_direct_analytical(key, value, g, beta, mode=mode, signature=signature, origin=origin)
    else:
        _warmup_legacy(key, value, g, beta, mode=mode, signature=signature, origin=origin)
    torch.npu.synchronize()
    _emit_ttx_warmup_marker(phase="SYNC", mode=mode, signature=signature, origin=origin)
