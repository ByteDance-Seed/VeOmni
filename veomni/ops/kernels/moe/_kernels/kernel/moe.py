...
def expert_histogram(input: torch.Tensor, num_bins: int) -> torch.Tensor:
    ...
    assert input.device.type in ("cuda", "mlu")
...
def moe_gather(x: torch.Tensor, index: torch.Tensor, out_dtype=None):
    assert x.device.type in ("cuda", "mlu") and index.device.type in ("cuda", "mlu")
    M, topk = index.shape
...
def moe_add_gather(x: torch.Tensor, y: torch.Tensor, index: torch.Tensor, out_dtype=None):
    assert x.device.type in ("cuda", "mlu") and y.device.type in ("cuda", "mlu") and index.device.type in ("cuda", "mlu")
