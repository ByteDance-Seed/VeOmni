...
"""Triton fused MoE forward with shared MoE-LoRA (Mode 2), non-EP, v4-split + v5-fused.

Numerics: identical to the eager wrappers at
``veomni.utils.moe_lora.LoraSharedExperts._eager_forward_fused_v5`` /
``_eager_forward_split_v4`` modulo fused/grouped-gemm rounding. Two
``autograd.Function`` flavours mirror the layout of the existing non-LoRA
fused MoE kernels:

* ``MergedFc1TritonFusedLoRAMoeExpertFunction`` — v5 fused layout. Mirrors
  ``MergedFc1TritonFusedMoeExpertFunction`` (single ``[E, 2I, H]`` fc1
  weight, single fused gate+up LoRA pair); LoRA delta added to ``fc1_output``
  before chunk/silu, and to ``fc2_output``.
* ``SplitFc1TritonFusedLoRAMoeExpertFunction`` — v4 split layout. Mirrors
  ``TritonFusedMoeExpertFunction`` (separate ``[E, I, H]`` gate / up
  weights, separate gate / up LoRA pairs); each LoRA delta added to its
  matching ``fc1_*_output`` before silu, and the down LoRA added to
  ``fc2_output``.

LoRA delta math (shared across both flavours):
...
Phase 4 scope:
    * Both layouts supported here. The wrapper picks one based on the
      detected layout of the experts module.
...
class MergedFc1TritonFusedLoRAMoeExpertFunction(torch.autograd.Function):
    """Fused MoE forward + shared MoE-LoRA (Mode 2), v5 fused layout, non-EP.
...
class SplitFc1TritonFusedLoRAMoeExpertFunction(torch.autograd.Function):
    """Fused MoE forward + shared MoE-LoRA (Mode 2), v4 split layout, non-EP.
    ...
    Inputs (forward):
        ...
        expert_index: ``[B*S, topk]`` selected expert ids per (token, slot).
        hidden_states: ``[B, S, H]`` (or ``[N, H]``) input activations.
        fc1_1_weight: ``[E, I, H]`` gate base weight.
        fc1_2_weight: ``[E, I, H]`` up base weight.
        fc2_weight: ``[E, H, I]`` down base weight.
        lora_a_gate: ``[r, H]``  shared LoRA A on gate.
        lora_b_gate: ``[I, r]``  shared LoRA B on gate.
        lora_a_up:   ``[r, H]``  shared LoRA A on up.
        lora_b_up:   ``[I, r]``  shared LoRA B on up.
        lora_a_down: ``[r, I]``  shared LoRA A on down.
        lora_b_down: ``[H, r]``  shared LoRA B on down.
        lora_scale_gate: scaling for the gate LoRA delta.
        lora_scale_up:   scaling for the up   LoRA delta.
    ...
    def forward(
        ...
        splits = expert_histogram(expert_index, num_experts)
        scatter_index = expert_index.flatten().argsort(stable=True).argsort().int().view(expert_index.shape)
        scatter_output = moe_scatter(hidden_states, scatter_index)  # [T, H]
        cumsum_t = torch.cumsum(splits, dim=0)
        ...
        # LoRA gate delta — shared across experts.
        tmp_g = torch.nn.functional.linear(scatter_output, lora_a_gate)  # [T, r]
        lora_delta_g = torch.nn.functional.linear(tmp_g, lora_b_gate) * lora_scale_gate  # [T, I]
        fc1_1_output = fc1_1_output + lora_delta_g
        ...
        # LoRA up delta — shared across experts.
        tmp_u = torch.nn.functional.linear(scatter_output, lora_a_up)  # [T, r]
        lora_delta_u = torch.nn.functional.linear(tmp_u, lora_b_up) * lora_scale_up  # [T, I]
        fc1_2_output = fc1_2_output + lora_delta_u
    ...
    def backward(ctx, grad_output):
        ...
        # Step 10: undo gather → grad on per-(token,slot) fc2 output.
        grad_fc2_output = moe_scatter(grad_output, scatter_index)  # [T, H]

        # ---- LoRA fc2 backward (closed form, identical to fused-v5 case). --
        grad_tmp_dn = torch.nn.functional.linear(grad_fc2_output, lora_b_dn.t()) * scale_dn  # [T, r]
        grad_lora_b_dn = grad_fc2_output.t().to(tmp_dn.dtype) @ tmp_dn * scale_dn  # [H, r]
        grad_lora_a_dn = grad_tmp_dn.t().to(fc1_weighted_output.dtype) @ fc1_weighted_output  # [r, I]
        grad_fc1_weighted_output_lora = torch.nn.functional.linear(grad_tmp_dn, lora_a_dn.t())  # [T, I]

        # Step 9 (base) — dgrad of fc2 wrt fc1_weighted_output.
        grad_fc1_weighted_output = group_gemm_same_nk(
            a=grad_fc2_output,
            b=fc2_weight,
            cumsum_M=cumsum_t,
        ...
        # Step 8: split routing-weight scale through fc1_weighted_output = fc1_activation * sgw.
        grad_fc1_activation = grad_fc1_weighted_output * scattered_gate_weight
        grad_scattered_gate_weight = torch.sum(fc1_activation * grad_fc1_weighted_output, dim=-1)
        grad_gate_weight = grad_scattered_gate_weight[scatter_index.flatten()]
        grad_gate_weight = grad_gate_weight.reshape(gate_weights.shape)

        # Step 7: chain through silu(gate) * up.
        fc1_1_activation = torch.ops.aten.silu(fc1_1_output)  # recompute (matches base fn)
        grad_fc1_1_activation = grad_fc1_activation * fc1_2_output  # [T, I]
        grad_fc1_2_output = fc1_1_activation * grad_fc1_activation  # [T, I]
        grad_fc1_1_output = torch.ops.aten.silu_backward(grad_fc1_1_activation, fc1_1_output)  # [T, I]

        # ---- LoRA fc1_1 (gate) backward (closed form). ------------------
        grad_tmp_g = torch.nn.functional.linear(grad_fc1_1_output, lora_b_gate.t()) * scale_gate  # [T, r]
        grad_lora_b_gate = grad_fc1_1_output.t().to(tmp_g.dtype) @ tmp_g * scale_gate  # [I, r]
        grad_lora_a_gate = grad_tmp_g.t().to(scatter_output.dtype) @ scatter_output  # [r, H]
        grad_scatter_output_lora_g = torch.nn.functional.linear(grad_tmp_g, lora_a_gate.t())  # [T, H]

        # ---- LoRA fc1_2 (up) backward (closed form). --------------------
        grad_tmp_u = torch.nn.functional.linear(grad_fc1_2_output, lora_b_up.t()) * scale_up  # [T, r]
        grad_lora_b_up = grad_fc1_2_output.t().to(tmp_u.dtype) @ tmp_u * scale_up  # [I, r]
        grad_lora_a_up = grad_tmp_u.t().to(scatter_output.dtype) @ scatter_output  # [r, H]
        grad_scatter_output_lora_u = torch.nn.functional.linear(grad_tmp_u, lora_a_up.t())  # [T, H]

        # Step 6 (base) — fc1_2 (up) dgrad+wgrad.
        ...
        # Step 4 (base) — fc1_1 (gate) dgrad+wgrad.
        ...
        # Step 3 — accumulate base + 2× LoRA dgrad on scatter_output, then gather.
        grad_scatter_output = (
            grad_scatter_output_1 + grad_scatter_output_2 + grad_scatter_output_lora_g + grad_scatter_output_lora_u
        )
        grad_hidden_states = moe_gather(grad_scatter_output, scatter_index)
        grad_hidden_states = grad_hidden_states.reshape(hidden_states.shape)
...
def group_gemm_fused_lora_moe_forward(
    ...
    *,
    # ── v5 fused layout (gate+up merged) ─────────────────────────────
    fc1_1_2_weight: Optional[torch.Tensor] = None,
    lora_a_gate_up: Optional[torch.Tensor] = None,
    lora_b_gate_up: Optional[torch.Tensor] = None,
    lora_scale_gate_up: Optional[float] = None,
    # ── v4 split layout (separate gate / up) ─────────────────────────
    fc1_1_weight: Optional[torch.Tensor] = None,
    fc1_2_weight: Optional[torch.Tensor] = None,
    lora_a_gate: Optional[torch.Tensor] = None,
    lora_b_gate: Optional[torch.Tensor] = None,
    lora_scale_gate: Optional[float] = None,
    lora_a_up: Optional[torch.Tensor] = None,
    lora_b_up: Optional[torch.Tensor] = None,
    lora_scale_up: Optional[float] = None,
) -> torch.Tensor:
    """Triton grouped-gemm fused MoE forward with shared MoE-LoRA (Mode 2).

    Dispatches based on which fc1 weights are provided:

    * **v5 fused** — pass ``fc1_1_2_weight`` (shape ``[E, 2I, H]``) plus the
      single fused gate+up LoRA pair (``lora_a_gate_up`` ``[r, H]``,
      ``lora_b_gate_up`` ``[2I, r]``, ``lora_scale_gate_up``).
    * **v4 split** — pass ``fc1_1_weight`` and ``fc1_2_weight`` (each
      ``[E, I, H]``) plus two separate LoRA pairs (``lora_a_gate`` ``[r, H]``,
      ``lora_b_gate`` ``[I, r]``, ``lora_scale_gate`` and the matching
      ``lora_*_up``).

    The ``lora_*_down`` triple is always required (positional). Both layouts
    share the same down LoRA shape (``[r, I]`` / ``[H, r]``).
    ...
    Mixing layout-specific kwargs (e.g. passing ``fc1_1_2_weight`` together
    with ``fc1_1_weight``) raises ``ValueError``.
    """
    ...
    fused_provided = fc1_1_2_weight is not None
    split_provided = fc1_1_weight is not None or fc1_2_weight is not None
    if fused_provided and split_provided:
        raise ValueError(
            "group_gemm_fused_lora_moe_forward: pass either v5 fused (fc1_1_2_weight + "
            "lora_*_gate_up) OR v4 split (fc1_1_weight + fc1_2_weight + lora_*_gate + "
            "lora_*_up), not both."
        )

    if fused_provided:
        if any(x is None for x in (lora_a_gate_up, lora_b_gate_up, lora_scale_gate_up)):
            raise ValueError(
                "group_gemm_fused_lora_moe_forward (v5 fused): lora_a_gate_up, "
                "lora_b_gate_up and lora_scale_gate_up are required when fc1_1_2_weight is given."
            )
    ...
    if split_provided:
        if fc1_1_weight is None or fc1_2_weight is None:
            raise ValueError(
                "group_gemm_fused_lora_moe_forward (v4 split): both fc1_1_weight and fc1_2_weight are required."
            )
        if any(x is None for x in (lora_a_gate, lora_b_gate, lora_scale_gate, lora_a_up, lora_b_up, lora_scale_up)):
            raise ValueError(
                "group_gemm_fused_lora_moe_forward (v4 split): lora_*_gate and lora_*_up triples are required."
            )
        return SplitFc1TritonFusedLoRAMoeExpertFunction.apply(
            num_experts,
            routing_weights,
            selected_experts,
            hidden_states,
    ...
    raise ValueError(
        "group_gemm_fused_lora_moe_forward: no fc1 weights provided. Pass either "
        "fc1_1_2_weight (v5 fused) or fc1_1_weight + fc1_2_weight (v4 split)."
    )
