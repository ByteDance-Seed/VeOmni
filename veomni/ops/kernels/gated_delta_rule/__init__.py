...
(``triton-ascend``, kept under ``_vendor/``).
...
KERNEL_REGISTRY.register(
    KernelSpec(
        ...
        hardware=HardwareRequirement(device_type="npu"),
        description="NPU vendored Triton causal conv1d (varlen-aware, speed-MM)",
    )
)
...
def _npu_chunk_gated_delta_rule_factory():
    ...
    backend.
    """
    from ._vendor.chunk_gated_delta_rule_mm import chunk_gated_delta_rule
...
KERNEL_REGISTRY.register(
    KernelSpec(
        ...
        hardware=HardwareRequirement(device_type="npu"),
        description="NPU vendored Triton chunk gated delta rule (varlen-aware, speed-MM)",
    )
)
