...
def dense_gca_attention_forward(
    ...
    *,
    ...
    ``attention_mask`` is a boolean edge mask (``True`` = attention allowed)
    shaped ``[batch, q_len, kv_len]`` or ``[batch, 1, q_len, kv_len]``;
    disallowed edges are filled with the dtype minimum (not ``-inf``) so a
    fully-masked row softmaxes to a uniform row instead of NaN. Grouped-query
    K/V heads are expanded to the Q head count via
    ``module.num_key_value_groups``. Materialises the full ``[batch, heads,
    q_len, kv_len]`` score matrix, so parity testing only.
    """
