...
"""gpt_oss MoE experts Quack implementation."""
...
def _segment_sum(values: torch.Tensor, cu_seqlens_m: torch.Tensor) -> torch.Tensor:
    """Sum ``values`` within each expert segment defined by ``cu_seqlens_m``."""
...
def _assert_contiguous(tensor: torch.Tensor, name: str) -> None:
    """Require ``tensor`` to be contiguous before a transposed quack GEMM view."""
...
def _gpt_oss_mlp_activation(gate_up: torch.Tensor, alpha: float, limit: float) -> torch.Tensor:
    """Interleaved clamped gate/up activation: ``(up+1) * gate * sigmoid(alpha*gate)``."""
...
def _gpt_oss_mlp_activation_backward(
    ...
    """Gradient of the interleaved gpt_oss MLP activation."""
    gate_raw = gate_up[..., ::2]
    up_raw = gate_up[..., 1::2]
...
class GptOssQuackFusedMoeExpertFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        """Non-EP fused experts: grouped GEMM, activation, routing multiply, gather."""
        from quack.gemm_interface import gemm

        from ..shared.dispatch import moe_gather

        cu_seqlens_m, A_idx, scatter_index = build_moe_indices(selected_experts, num_experts)
    ...
    def backward(ctx, grad_output):
        """Non-EP grads for hidden, routing weights, and expert projections."""
        from quack.gemm_interface import gemm
...
class EPGptOssQuackGroupGemm(torch.autograd.Function):
    ...
    def forward(
        ...
        """EP-local expert GEMMs on already-permuted tokens."""
        from quack.gemm_interface import gemm
    ...
    @staticmethod
    def backward(ctx, grad_output):
        """EP-local grads for permuted tokens and expert projections."""
        from quack.gemm_interface import gemm
...
def wrapper(
    ...
    *,
    ...
    """Call the GPT-OSS Quack fused MoE Function."""
