...
"""Shared cuDNN Frontend DeepSeek Sparse Attention helpers."""
...
def _local_topk_to_global(topk_indices: torch.Tensor, seqlen_k: int) -> torch.Tensor:
    ...
    topk_i32 = topk_indices if topk_indices.dtype == torch.int32 else topk_indices.to(torch.int32)
...
def check_sparse_mla_forward_compatible(
    q_pe: torch.Tensor,
    k_pe: torch.Tensor,
    ...
    """Validate the DeepSeek sparse MLA forward contract shared by FlashMLA and FA4."""
    if q_pe.dim() != 4:
    ...
    if kv_cache.shape[-1] != 512:
        return False, f"DeepSeek sparse MLA forward currently requires value dim 512, got {kv_cache.shape[-1]}"
    packed_dim = q_nope.shape[-1] + q_pe.shape[-1]
    if packed_dim != 576:
        return False, f"DeepSeek sparse MLA forward currently requires packed q/k dim 576, got {packed_dim}"
    if k_pe.shape[2] != 1:
        return False, f"DeepSeek sparse MLA forward wrapper requires MQA K/V with H_kv=1, got {k_pe.shape[2]}"
    if gather_kv_indices is not None:
        ...
        if gather_kv_indices.shape[-1] % 128 != 0:
            return (
                False,
                f"DeepSeek sparse MLA forward requires topk to be a multiple of 128, got {gather_kv_indices.shape[-1]}",
            )
...
def pack_sparse_mla_tensors_for_backward(
    q_pe: torch.Tensor,
    k_pe: torch.Tensor,
    kv_cache: torch.Tensor,
    q_nope: torch.Tensor,
) -> dict[str, torch.Tensor]:
    """Pack sparse MLA tensors into cuDNN FE DSA backward's unified layout."""
    compatible, reason = check_sparse_mla_forward_compatible(q_pe, k_pe, kv_cache, q_nope, gather_kv_indices=None)
    if not compatible:
