...
def _dsv4_rotary_kernel():
    """Compile and cache the Triton DeepSeek-V4 RoPE kernel."""
    import triton
    ...
    def kernel(
        ...
        x = tl.load(
            X + rm[:, None] * stride_x_seqlen + rd[None, :] * stride_x_headdim, mask=mask_m & in_head, other=0.0
        )
        x_swap = tl.load(
...
def forward(x: Tensor, cos: Tensor, sin: Tensor, *, unsqueeze_dim: int = 1) -> tuple[Tensor, SavedState]:
    ...
    output = _rotary_launch(x, cos, sin, conjugate=False)
