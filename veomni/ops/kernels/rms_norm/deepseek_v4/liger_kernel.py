...
"""DeepSeek-V4 RMSNorm Liger adapter (offset 0, gemma casting)."""
...
def forward(x: Tensor, weight: Tensor, *, eps: float) -> tuple[Tensor, SavedState]:
    """Liger fused RMSNorm with an fp32 affine multiply and offset 0."""
    if x.numel() == 0:
...
def backward(grad_output: Tensor, saved: SavedState) -> tuple[Tensor, Tensor]:
    """Return ``(grad_x, grad_weight)``. Empty inputs reuse eager backward."""
    meta = saved.metadata
