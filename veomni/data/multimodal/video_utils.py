...
def _load_and_process_video_with_codec(video_input: VideoInput, use_audio_in_video: bool = True, **kwargs):
    ...
    if isinstance(video_input, list):
        ...
        # Don't process audio here - let fetch_videos handle it uniformly
        return video, audio, audio_fps

    # --- From here, video_input is either str (path/URL) or bytes ---

    # --- 1. Video Handling (TorchCodec) ---
    # TorchCodec should support URLs directly, but if it fails, we'll download using ffmpeg
    try:
        decoder = VideoDecoder(video_input, device="cpu", num_ffmpeg_threads=0)
    except Exception as e:
        # If video_input is a URL and direct decoding failed, try downloading first
    ...
    # Decode all frames or a reasonable subset to avoid edge cases
    # Use a mild safety margin: skip the last 1-2 frames as some videos report inaccurate total_frames
    max_decodable_frame = max(0, total_frames - 2)

    # Decode frames uniformly across the video
    # We'll decode all frames (or up to a reasonable limit) and let smart_video_nframes do the final sampling
    # This avoids torchcodec's edge case errors when requesting specific frame indices near the end
    initial_sample_count = min(total_frames, 100)  # Sample up to 100 frames or all if fewer

    if initial_sample_count <= 1:
        indices = [0]
    else:
        indices = np.linspace(0, max_decodable_frame, initial_sample_count, dtype=int).tolist()
    ...
    try:
        ...
        if "Requested next frame" in str(e) or "End of stream" in str(e):
            logger.warning(f"TorchCodec decoding edge case caught: {e}. Retrying with first frame only.")
            # Strategy: if failed, only get frame 0, decoder will be closed and we'll use this single frame
            try:
    ...
    # Apply resize first, then let smart_video_nframes handle sampling and padding
    resized_frames = smart_resize(frames, **kwargs)
    # Now apply smart_video_nframes to get the final frame count with proper sampling
    final_frames = smart_video_nframes(resized_frames, video_fps, **kwargs)
    ...
    return final_frames, audio, audio_fps
...
def fetch_videos(videos: List[VideoInput], **kwargs):
    ...
    # Audio is extracted but not resampled in _load_and_process_video_with_codec
    processed_audio_inputs = [
        smart_audio_nframes(audio, audio_fps, **kwargs) if audio is not None and audio_fps is not None else None
