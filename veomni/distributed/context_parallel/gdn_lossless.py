...
def attach_state_dependency(output: Tensor, state: Tensor) -> Tensor:
    ...
    if state.numel() == 0:
        return output
    return output + state.reshape(-1)[0].to(dtype=output.dtype) * 0
...
def make_state_participation(*tensors: Tensor) -> Tensor:
    ...
    participation = tensors[0].reshape(-1)[0] * 0
    for tensor in tensors[1:]:
        participation = participation + tensor.reshape(-1)[0] * 0
...
def exchange_conv_halo(
    ...
    *,
    ...
    participation = payload.reshape(-1)[0] if payload.numel() else owned.new_zeros(())
    ...
    with_halo = with_halo + sent.reshape(-1)[0].to(dtype=with_halo.dtype) * 0
