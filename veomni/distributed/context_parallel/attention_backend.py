...
_NPU_CAUSAL_MASK_CACHE: dict[torch.device, Tensor] = {}


def _require_torch_npu():
    if not is_torch_npu_available():
        raise RuntimeError("The NPU Ring-attention backend requires torch_npu.")
    import torch_npu

    return torch_npu


def _npu_causal_mask(device: torch.device) -> Tensor:
    mask = _NPU_CAUSAL_MASK_CACHE.get(device)
    if mask is None:
        mask = torch.ones((2048, 2048), dtype=torch.bool, device=device).triu(diagonal=1)
        _NPU_CAUSAL_MASK_CACHE[device] = mask
    return mask
...
def npu_attention_forward(
    ...
    *,
    ...
    if causal:
        atten_mask = _npu_causal_mask(query.device)
        next_tokens = 0
        sparse_mode = 3
...
def torch_packed_causal_attention(
    ...
    *,
    ...
    cu = _validate_torch_packed_cu_seqlens(cu_seqlens, sequence_length=query.size(2))
    outs = []
...
def _validate_torch_packed_cu_seqlens(cu_seqlens: Tensor, *, sequence_length: int) -> list[int]:
    """Validate packed offsets before an empty segment reaches torch attention."""
    if cu_seqlens.ndim != 1 or cu_seqlens.dtype not in (torch.int32, torch.int64):
        raise ValueError(
            "torch_packed_causal_attention expects a 1-D int32/int64 cu_seqlens tensor, "
            f"got shape={tuple(cu_seqlens.shape)} dtype={cu_seqlens.dtype}"
        )
    raw_points = cu_seqlens.detach().cpu().tolist()
    if any(isinstance(point, bool) or not isinstance(point, Integral) for point in raw_points):
        raise TypeError("Packed causal attention cu_seqlens must contain integers.")
    points = [int(point) for point in raw_points]
    if len(points) < 2:
        raise ValueError("Packed causal attention cu_seqlens must contain at least [0, sequence_length].")
    if points[0] != 0:
        raise ValueError(f"Packed causal attention cu_seqlens must start at zero, got {points[0]}.")
    if any(end <= start for start, end in zip(points, points[1:])):
        raise ValueError("Packed causal attention cu_seqlens must be strictly increasing.")
    if points[-1] != int(sequence_length):
        raise ValueError(
            "Packed causal attention cu_seqlens must end at local sequence length "
            f"{sequence_length}, got {points[-1]}."
        )
    return points
