...
"""Sharding helpers for Ring/Hybrid and DeepSeek-V4 context parallelism."""
...
def balanced_cp_to_rank_major(tensor: Tensor, cp_size: int, dim: int = 0) -> Tensor:
    ...
    num_chunks = 2 * cp_size
    if tensor.size(dim) % num_chunks != 0:
        raise ValueError(f"Sequence length ({tensor.size(dim)}) must be divisible by 2 * cp_size ({num_chunks}).")
...
def hybrid_cp_slice(
    ...
    if cp_shard.size(dim) % ulysses_size != 0:
        raise ValueError(
            f"CP-local sequence length ({cp_shard.size(dim)}) must be divisible by ulysses_size ({ulysses_size})."
        )
