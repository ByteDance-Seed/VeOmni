...
def _output_scale(statistic: Tensor, output: Tensor) -> Tensor:
    ...
    if scale.ndim != output.ndim:
        raise ValueError(f"Softmax statistic rank ({scale.ndim}) must match attention output rank ({output.ndim}).")
...
def merge_attention_blocks(
    ...
    if previous_softmax_max.shape != current_softmax_max.shape:
        raise ValueError(
            f"Softmax max shapes must match, got {previous_softmax_max.shape} and {current_softmax_max.shape}."
        )
    if previous_softmax_sum.shape != current_softmax_sum.shape:
        raise ValueError(
            f"Softmax sum shapes must match, got {previous_softmax_sum.shape} and {current_softmax_sum.shape}."
        )
