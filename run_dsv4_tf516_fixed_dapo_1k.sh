...
torchrun --nnodes=1 --nproc-per-node=16 --node-rank=0 --master-addr=127.0.0.1 --master-port=64470 "${common_args[@]}" --model.ops_implementation.dsa_indexer_implementation=npu --model.ops_implementation.dsa_attention_implementation=npu --model.ops_implementation.mhc_implementation=npu --train.checkpoint.output_dir=dsv4_tf516_fixed_fused_1k > dsv4_242_tf516_fixed_fused_1000step_20260919.log 2>&1
export HCCL_NPU_SOCKET_PORT_RANGE=59800-59850
torchrun --nnodes=1 --nproc-per-node=16 --node-rank=0 --master-addr=127.0.0.1 --master-port=64480 "${common_args[@]}" --model.ops_implementation.dsa_indexer_implementation=eager --model.ops_implementation.dsa_attention_implementation=eager --model.ops_implementation.mhc_implementation=eager --train.checkpoint.output_dir=dsv4_tf516_fixed_eager_1k > dsv4_242_tf516_fixed_eager_1000step_20260919.log 2>&1
