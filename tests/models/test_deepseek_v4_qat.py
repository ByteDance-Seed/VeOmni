...
def _qat_slot(qat_implementation: str) -> OpsConfigSlot:
    """An ``OpsConfigSlot`` already holding ``qat_implementation``.

    ``OpsConfigSlot`` takes no constructor default -- an unbound slot always
    reads ``"eager"`` -- so a test that wants a specific recipe has to bind one,
    which is also the only path the runtime uses.
    """
    slot = OpsConfigSlot("qat_implementation")
    slot.bind(SimpleNamespace(qat_implementation=qat_implementation))
    return slot
...
def test_qat_is_off_until_asked_for(modeling):
    ...
    Two independent defaults carry that, and neither is the other's. An unbound
    ``OpsConfigSlot`` reads ``"eager"`` -- the registry's shared fallback, not
    this field's own value -- while the config a real run binds from defaults to
    ``"none"``. Both are ``!= "fp8_blockwise"``, which is the only thing the
    helpers test, so pin each default at its source rather than assert the
    inequality, which no reachable value could fail.

    The module-level slot is checked for the weak property on purpose: another
    test module in the same session may have built a model and bound it, and
    what matters here is that doing so cannot turn QAT on by itself.
    """
    ...
    assert OpsConfigSlot("qat_implementation").value == "eager"
    assert OpsImplementationConfig().qat_implementation == "none"


def test_qat_slot_binds_from_the_ops_config(monkeypatch):
    from veomni.arguments.arguments_types import OpsImplementationConfig
    from veomni.models.auto import _bind_veomni_ops
    from veomni.utils import device as device_utils

    # ``OpsImplementationConfig.__post_init__`` refuses ``fp8_blockwise`` off an
    # SM90+ CUDA host, and would raise here before the bind step this test
    # exists to exercise: GPU CI is L20 (SM89) and NPU CI is 910b. Binding is
    # hardware-independent, so stub the probe rather than skip the test.
    monkeypatch.setattr(device_utils, "IS_CUDA_AVAILABLE", True)
    monkeypatch.setattr(device_utils, "get_gpu_compute_capability", lambda *_: 90)
