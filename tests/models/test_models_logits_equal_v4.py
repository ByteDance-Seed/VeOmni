"""Bitwise logits-equal tests for transformers-v4-only models.

Counterpart to ``test_models_logits_equal.py`` (the v5 default lane). Models
that have not yet been migrated to the v5 patchgen pipeline (currently
``llama3_1`` and ``qwen2_5_omni``) are exercised here against the
``transformers-v4-legacy`` extra installed in the dedicated
``gpu_unit_tests_v4`` job. The v4 test infrastructure (``hf_unpatch`` to
restore pristine HF classes between cases, manual env wiring for the
single-rank process group) is unique to this lane — v5 doesn't need it
because the patchgen modeling is self-contained.

When a model gains a v5 patchgen migration it should move to
``test_models_logits_equal.py`` and stop being collected here.
"""
...
# Skip the whole module on transformers >= 5.0.0; the listed cases are
# v4-only by design (see CASES below). ``allow_module_level=True`` is
# safe because the v5 default lane runs ``test_models_logits_equal.py``
# instead, so this skip never produces a "0 collected" pytest exit code 5
# in the v5 lane.
if is_transformers_version_greater_or_equal_to("5.0.0"):
    pytest.skip(
        "test_models_logits_equal_v4.py only covers v4-only models; run on the transformers-v4-legacy lane instead.",
...
# gpu_patch.py. VEOMNI_USE_FUSED_KERNELS=0 additionally disables fused-kernel
# paths (Triton RoPE + batch-invariant RMSNorm) that some models default to
# when Liger is off.
...
CASES = [
    ...
    # ---- text-only causal-LM ----
    # eager + fp32
    Case("llama3_1-toy-eager", _toy("llama31_toy")),
...
def _single_rank_process_group():
    ...
    The module-level v5 skip already handles the version gate, so we only
    short-circuit when no accelerator is available here.
    """
...
_CAUSAL_LM_MODEL_TYPES = frozenset({"qwen2", "qwen3", "qwen3_moe", "llama", "seed_oss"})
...
def test_logits_bitwise_equal(case: Case):
    ...
    process-wide.
    """
    # Module-level v5 skip already gates the transformers version; only
    # the runtime accelerator and case-local preconditions remain here.
    if not IS_CUDA_AVAILABLE:
