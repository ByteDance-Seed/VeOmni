...
def test_registered_impls():
    assert KERNEL_REGISTRY.list_registered("load_balancing_loss", "standard") == ["eager", "triton"]


def test_eager_uniform_distribution_is_well_conditioned():
    torch.manual_seed(0)
    gate_logits = torch.randn(3 * 1024, 8)
    output = resolve_kernel("load_balancing_loss", "standard", "eager").wrapper(
...
def test_eager_all_masked_returns_zero_with_zero_grad():
    ...
    output = resolve_kernel("load_balancing_loss", "standard", "eager").wrapper(gate_logits, attention_mask, top_k=2)
...
def test_eager_forward_matrix_matches_hf(num_experts, top_k, num_layers, batch, seq_len, use_mask):
    ...
    actual = resolve_kernel("load_balancing_loss", "standard", "eager").wrapper(
        concatenated, attention_mask, top_k=top_k
    )
...
def test_triton_all_masked_returns_zero_with_zero_grad():
    ...
    output = resolve_kernel("load_balancing_loss", "standard", "triton").wrapper(gate_logits, attention_mask, top_k=2)
    assert output.item() == 0.0
    output.backward()
    assert torch.count_nonzero(gate_logits.grad) == 0
...
def test_triton_is_deterministic(num_experts, top_k, num_layers, batch_size, seq_len, use_mask):
    ...
    if use_mask:
        ...
        attention_mask = _empty_mask("cuda")
    triton = resolve_kernel("load_balancing_loss", "standard", "triton").wrapper
