...
_WRAPPER = "veomni.kernels._kernels.dsa.sparse_mqa_target"
_VENDOR_TARGET = "veomni.kernels._kernels.dsa.vendor.tilelang_sparse_mla_target"
...
def test_wrapper_is_not_a_registered_kernel():
    ...
    with pytest.raises(KeyError, match="sparse_mqa_target"):
        resolve_kernel("sparse_mqa_target", "standard", "tilelang")
    with pytest.raises(KeyError, match="sparse_mqa_target_fwd"):
        resolve_kernel("sparse_mqa_target_fwd", "standard", "tilelang")
...
def test_wrapper_rejects_pre_sm90_before_import(monkeypatch):
    ...
    sys.modules.pop(_VENDOR_TARGET, None)
    import veomni.kernels._kernels.dsa.sparse_mqa_target as target
...
def test_target_kernel_matches_reference(heads, c):
    ...
    from veomni.kernels._kernels.dsa.vendor.tilelang_sparse_mla_fwd import sparse_mqa_fwd_interface
    ...
    actual = sparse_mqa_target_fwd(q, kv, topk[:, :, w:].contiguous(), lse, sm_scale=scale)
...
def test_target_kernel_zeroes_invalid_slots():
    ...
    _, lse = sparse_mqa_fwd_interface(q, kv, sink, topk, sm_scale=scale)
    target = sparse_mqa_target_fwd(q, kv, topk[:, :, w:].contiguous(), lse, sm_scale=scale)
...
def test_target_kernel_all_invalid_compressed_row_is_zero():
    ...
    _, lse = sparse_mqa_fwd_interface(q, kv, sink, topk, sm_scale=scale)
    raw = sparse_mqa_target_fwd(q, kv, topk[:, :, w:].contiguous(), lse, sm_scale=scale)
...
def test_target_kernel_rejects_head_counts_the_interface_cannot_emit():
    ...
    """
    _require_tilelang_cuda()
    from veomni.kernels._kernels.dsa.vendor.tilelang_sparse_mla_target import sparse_mqa_target
...
def test_target_kernel_rejects_non_bfloat16_inputs():
    ...
    with pytest.raises(RuntimeError, match="bfloat16-only, got q"):
        sparse_mqa_target_fwd(q.to(torch.float16), kv, topk, lse)
    with pytest.raises(RuntimeError, match="bfloat16-only, got kv"):
        sparse_mqa_target_fwd(q, kv.to(torch.float16), topk, lse)
...
def test_sparse_attn_returns_non_differentiable_lse():
    ...
    sparse_attn_tilelang = VeomniKernel("dsa_attention", "deepseek_v4", "tilelang")
