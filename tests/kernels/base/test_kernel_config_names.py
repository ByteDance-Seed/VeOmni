...
"""Check that in-tree YAML kernel names resolve to registered implementations."""
...
def _registered_implementations(kernel: str) -> set[str]:
    """Return implementation names registered across every kernel variant and device."""
    return {key[2] for key in KERNEL_REGISTRY._entries if key[0] == kernel}


def test_yaml_kernel_implementation_names_are_registered():
    ...
    for root in (REPO_ROOT / "configs", REPO_ROOT / "tests"):
        for path in sorted(root.rglob("*.yaml")):
            ...
            for field, implementation in implementations.items():
                kernel = _FIELD_TO_KERNEL.get(field)
                if kernel is None:
                    invalid.append(f"{path.relative_to(REPO_ROOT)}: unmapped implementation field {field!r}")
                    continue
                if implementation in _registered_implementations(kernel):
                    continue
                invalid.append(f"{path.relative_to(REPO_ROOT)}: {field}={implementation!r} ({kernel})")

    assert not invalid, "YAML kernel implementation names are not registered:\n" + "\n".join(invalid)
