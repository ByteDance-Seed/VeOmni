...
injection) and only adds a per-step loss and timing logger, so the e2e and
throughput smokes can assert finite losses and compute steady-state stats.
Unlike train_vlm_test.py it does not stub the data transform.
"""
...
class _LogDictSaveCallback(Callback):
    ...
    def on_train_begin(self, state: TrainerState, **kwargs) -> None:
        # No-op: kept as an explicit hook point for startup telemetry.
        return
    ...
    def on_step_end(self, state: TrainerState, loss: float, loss_dict: Dict[str, float], grad_norm: float, **kwargs):
        ...
        # EnvironMeterCallback.on_step_end publishes step_env_metrics (mfu,
        # tokens_per_second, tflops, memory) BEFORE this runs, because base
        # callbacks fire first via super().on_step_end.
