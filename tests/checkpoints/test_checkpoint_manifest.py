...
"""Checkpoint manifest (same-topology gate) + full RNG snapshot, CPU-only."""
...
def test_identical_manifests_are_compatible():
    a = _manifest(extra={"model_identity": "abc"}, soft={"train_seed": 0})
    b = _manifest(extra={"model_identity": "abc"}, soft={"train_seed": 0})
...
def test_extra_hash_change_is_hard_fail():
    saved = _manifest(extra={"model_identity": "policy-A", "component_policy": {"vae_encoder": "frozen"}})
    current = _manifest(extra={"model_identity": "policy-B", "component_policy": {"vae_encoder": "frozen"}})
    hard, _ = validate_checkpoint_manifest(saved, current)
    assert any("model_identity" in r for r in hard)
...
def test_write_read_round_trip(tmp_path):
    manifest = _manifest(extra={"model_identity": "abc"})
...
def test_rng_snapshot_restore_reproduces_stream():
    ...
    # Advance the generators before restoring, so a no-op restore would fail.
    torch.rand(10)
