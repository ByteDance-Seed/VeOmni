...
import yaml
from checkpoint_verification_utils import verify_hf_checkpoint
...
def test_trainer_saveload_ep8():
    ...
    # Verify HF checkpoint structure (no weight comparison since we don't have original state dict in test context)
    logger.info("Verifying HF checkpoint structure...")
    assert verify_hf_checkpoint(hf_output_dir, verify_structure=True, verify_weights=False), (
...
def test_trainer_saveload_ep4():
    ...
    # Verify HF checkpoint structure
    logger.info("Verifying HF checkpoint structure...")
    assert verify_hf_checkpoint(hf_output_dir, verify_structure=True, verify_weights=False), (
        "HF checkpoint verification failed"
    )
