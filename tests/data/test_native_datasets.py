...
def run_data_test():
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        logger.info(f"[rank{rank}] train steps: {args.train.train_steps}")
        for _ in range(start_step, args.train.train_steps):
            ...
            logger.info(f"[rank{rank}] global step: {global_step}")
            logger.info(f"[rank{rank}] batch size: {len(micro_batches)}")
