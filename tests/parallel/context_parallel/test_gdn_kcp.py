...
def _install_direct_warmup_fakes(monkeypatch, *, fail_on=None):
    ttx_module._TTX_FORWARD_BACKWARD_WARMUP_CACHE.clear()
    ttx_module._TTX_WARMUP_MARKERS.clear()
    monkeypatch.setenv("VEOMNI_TTX_WARMUP_MODE", "direct_analytical")
    ...
    def fake_forward(*args, **kwargs):
        ...
        if fail_on == "forward":
            raise RuntimeError("synthetic warmup forward failure")
        return torch.zeros(2, 2, 4, 8)

    def fake_backward(*args, **kwargs):
        backward_calls.append(True)
        if fail_on == "backward":
            raise RuntimeError("synthetic warmup backward failure")
        zeros = torch.zeros(1)
        return zeros, zeros, zeros, zeros

    def forbidden_autograd_grad(*args, **kwargs):
        raise AssertionError("torch.autograd.grad must not be called")

    monkeypatch.setattr(ttx_module, "_ttx_local_affine_summary_fwd", fake_forward)
    monkeypatch.setattr(ttx_bwd_module, "ttx_local_affine_analytical_bwd", fake_backward)
    monkeypatch.setattr(ttx_module.torch.autograd, "grad", forbidden_autograd_grad)
    return forward_calls, backward_calls, synchronize_calls


def test_ttx_direct_analytical_warmup_skips_autograd_and_caches_only_success(monkeypatch):
    forward_calls, backward_calls, synchronize_calls = _install_direct_warmup_fakes(monkeypatch)
    key, value, g, beta = _inputs(tokens=0)
    with torch.no_grad():
        ttx_module.warmup_ttx_bc8_m1_forward_backward(key, value, g, beta)
        ttx_module.warmup_ttx_bc8_m1_forward_backward(key, value, g, beta)

    phases = [row["phase"] for row in ttx_module._TTX_WARMUP_MARKERS]
    hashes = {row["sig_hash"] for row in ttx_module._TTX_WARMUP_MARKERS}
    assert forward_calls == [True]
    assert backward_calls == [True]
    assert synchronize_calls == [True]
    assert phases == ["FWD_ENTER", "FWD_EXIT", "BWD_ENTER", "BWD_EXIT", "SYNC", "CACHE_HIT"]
    assert {row["origin"] for row in ttx_module._TTX_WARMUP_MARKERS} == {"runtime"}
    assert len(hashes) == 1
    ...
    with torch.inference_mode():
        ttx_module.warmup_ttx_bc8_m1_forward_backward(key, value, g, beta)
    assert forward_calls == [True, True]
    assert backward_calls == [True, True]
    assert synchronize_calls == [True, True]
    assert [row["phase"] for row in ttx_module._TTX_WARMUP_MARKERS] == [
        "FWD_ENTER",
        "FWD_EXIT",
        "BWD_ENTER",
        "BWD_EXIT",
...
def test_ttx_warmup_records_build_pre_fsdp_origin(monkeypatch):
    _install_direct_warmup_fakes(monkeypatch)
    key, value, g, beta = _inputs(tokens=0)
    ttx_module.warmup_ttx_bc8_m1_forward_backward(key, value, g, beta, origin="build_pre_fsdp")
    ttx_module.warmup_ttx_bc8_m1_forward_backward(key, value, g, beta)
    assert [row["origin"] for row in ttx_module._TTX_WARMUP_MARKERS] == [
        "build_pre_fsdp",
        "build_pre_fsdp",
        "build_pre_fsdp",
        "build_pre_fsdp",
...
@pytest.mark.parametrize("fail_on", ["forward", "backward"])
def test_ttx_direct_analytical_warmup_does_not_cache_partial_failure(monkeypatch, fail_on):
    _install_direct_warmup_fakes(monkeypatch, fail_on=fail_on)
    key, value, g, beta = _inputs(tokens=0)
    with pytest.raises(RuntimeError, match=f"synthetic warmup {fail_on} failure"):
        ttx_module.warmup_ttx_bc8_m1_forward_backward(key, value, g, beta)
    assert not ttx_module._TTX_FORWARD_BACKWARD_WARMUP_CACHE
    assert "CACHE_HIT" not in {row["phase"] for row in ttx_module._TTX_WARMUP_MARKERS}


def test_ttx_warmup_unknown_mode_is_fail_closed(monkeypatch):
    ttx_module._TTX_FORWARD_BACKWARD_WARMUP_CACHE.clear()
    monkeypatch.setenv("VEOMNI_TTX_WARMUP_MODE", "nested_autograd")
    monkeypatch.setattr(ttx_module, "validate_ttx_bc8_m1_inputs", lambda *args, **kwargs: None)
    with pytest.raises(RuntimeError, match="unknown TTX warmup mode"):
        ttx_module.warmup_ttx_bc8_m1_forward_backward(*_inputs(tokens=0))
    assert not ttx_module._TTX_FORWARD_BACKWARD_WARMUP_CACHE


def test_ttx_legacy_warmup_still_uses_autograd_and_caches_only_success(monkeypatch):
    ttx_module._TTX_FORWARD_BACKWARD_WARMUP_CACHE.clear()
    ttx_module._TTX_WARMUP_MARKERS.clear()
    monkeypatch.setenv("VEOMNI_TTX_WARMUP_MODE", "legacy")
    monkeypatch.setattr(ttx_module, "validate_ttx_bc8_m1_inputs", lambda *args, **kwargs: None)
    synchronize_calls = []
    summary_calls = []
    ...
    def fake_summary(key, value, g, beta, **kwargs):
        summary_calls.append(torch.is_grad_enabled())
    ...
    assert synchronize_calls == [True]
    assert [row["phase"] for row in ttx_module._TTX_WARMUP_MARKERS][-1] == "CACHE_HIT"
    assert len(ttx_module._TTX_FORWARD_BACKWARD_WARMUP_CACHE) == 1
