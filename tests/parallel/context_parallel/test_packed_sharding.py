...
def test_packed_cp_partition_keeps_sample_boundaries_cp2():
    ...
    seq = torch.arange(32)
    p0 = build_packed_context_parallel_partition(cu, cp_size=2, cp_rank=0)
    p1 = build_packed_context_parallel_partition(cu, cp_size=2, cp_rank=1)
    ...
    torch.testing.assert_close(apply_packed_context_parallel_partition(seq, p0, dim=0), expected0)


def test_packed_partition_rejects_non_divisible_sample():
    cu = torch.tensor([0, 15, 32], dtype=torch.int32)
    with pytest.raises(ValueError, match="divisible"):
        build_packed_context_parallel_partition(cu, cp_size=2, cp_rank=0)


def test_hybrid_packed_partition_u4_cp2():
    cu = torch.tensor([0, 64], dtype=torch.int32)  # one sample, multiple=16
    parts = [
        build_packed_context_parallel_partition(cu, cp_size=2, cp_rank=cp, ulysses_size=4, ulysses_rank=u)
        for cp in range(2)
        for u in range(4)
    ]
...
def test_simulate_packed_ring_matches_packed_dense_and_blocks_cross_sample():
    ...
    ring = simulate_packed_ring_causal_attention(query, key, value, cu, cp_size=cp_size, softmax_scale=head_dim**-0.5)
    ...
    s0 = dense_causal_attention(query[:, :, :32], key[:, :, :32], value[:, :, :32], softmax_scale=head_dim**-0.5)
...
def test_ulysses_rank_major_reorder_roundtrip():
    ...
    sample_major = reorder_ulysses_rank_major_to_sample_major(rank_major, local_cu, ulysses_size=2, sequence_dim=1)
    ...
    back = reorder_sample_major_to_ulysses_rank_major(sample_major, local_cu, ulysses_size=2, sequence_dim=1)
    torch.testing.assert_close(back, rank_major)
    global_cu = torch.tensor([0, 16, 32], dtype=torch.int32)
    local_cu = ulysses_local_cu_from_global(global_cu, cp_size=2, ulysses_size=2)
    torch.testing.assert_close(local_cu, torch.tensor([0, 4, 8], dtype=torch.int32))


@pytest.mark.parametrize(
    ("points", "match"),
    [
        ([0, 4, 4, 8], "strictly increasing"),
        ([0, 6, 5, 8], "strictly increasing"),
        ([1, 8], "start at zero"),
        ([0, 7], "end at local sequence length"),
        ([0], "at least"),
    ],
)
def test_packed_dense_attention_rejects_invalid_cu_seqlens(points, match):
    query = torch.randn(1, 2, 8, 4)
    key = torch.randn(1, 2, 8, 4)
    value = torch.randn(1, 2, 8, 4)
    with pytest.raises(ValueError, match=match):
        torch_packed_causal_attention(query, key, value, torch.tensor(points, dtype=torch.int32))
