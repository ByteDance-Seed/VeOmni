...
def test_simulate_ring_matches_dense_causal_forward(
    ...
    actual = simulate_ring_causal_attention(query, key, value, cp_size=cp_size, softmax_scale=scale, backend="torch")
...
def test_simulate_ring_matches_dense_causal_gradients_gqa():
    ...
    ring_out = simulate_ring_causal_attention(query, key, value, cp_size=cp_size, softmax_scale=scale, backend="torch")
...
def test_ring_backend_selection_fails_closed():
    with pytest.raises(ValueError, match="Unknown Ring-attention backend"):
        _resolve_backend("experimental", torch.device("cpu"))
    with pytest.raises(ValueError, match="requires NPU tensors"):
        _resolve_backend("npu", torch.device("cpu"))
...
def test_packed_ring_cu_seqlens_fail_closed(cu, match):
    with pytest.raises(ValueError, match=match):
        _validate_packed_cu_seqlens(cu, sequence_length=8)

    with pytest.raises(TypeError, match="must contain integers"):
        _validate_packed_cu_seqlens([0, True, 8], sequence_length=8)
