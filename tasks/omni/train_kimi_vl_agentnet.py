...
def process_sample(
    ...
    **kwargs,
    ...
    conversations = conv_preprocess(source, sample, **kwargs)
    ...
    for traj in sample["traj"][-WINDOW:]:
        image = traj['image']
        folder = "ubuntu_images" if sample['platform']=="ubuntu" else "win_mac_images"
        images.append(Image.open(f"data/AgentNet/{folder}/images/" + image).convert("RGB"))
    ...
    image_token_num = image_grid_hws.prod(dim=-1) // merge_length
    token_num_inputs["image"] = image_token_num # 如果图的数量太多，截断后会出现token数量和featur数量不匹配的问题

    loss_mask = [0 for _ in range(len(conversations))]
    loss_mask[-1] = 1
    tokenized_example = chat_template.encode_messages(conversations, token_num_inputs, max_seq_len=max_seq_len, loss_mask=loss_mask)
    ...
    image_mask = tokenized_example["input_ids"] == IMAGE_INPUT_INDEX
    tokenized_example["input_ids"][image_mask] = 163605 # TODO: config.media_placeholder_token_id
...
def main():
    ...
    model = build_foundation_model(
        ...
        attn_implementation=args.model.attn_implementation, # this is not working for kimi-vl
        config_kwargs={"attn_implementation":args.model.attn_implementation},
        moe_implementation=args.model.moe_implementation,
        init_device=args.train.init_device,
        force_use_huggingface=args.model.force_use_huggingface,
    )
    # TODO: temp fix, dont know why this load error happens
    # for i in range(model.config.text_config.num_hidden_layers):
    #     full_inv_freq = model.language_model.model.layers[i].self_attn.rotary_emb.inv_freq
    #     model.language_model.model.layers[i].self_attn.rotary_emb.inv_freq = full_inv_freq[::2]
    ...
    if args.data.dataloader_type == "native":
    ...
    elif args.data.dataloader_type == "energon":
        try:
            from megatron.energon import get_train_dataset, get_loader, WorkerConfig
        except ImportError:
            logger.error("Need megatron.energon with args.data.dataloader_type == `energon`")
        logger.warning_rank0("Igonred args with args.data.dataloader_type == `energon`:\n\
            ...
            args.train.rmpad,\n\
            args.train.rmpad_with_pos_ids,\n\
            args.train.bsz_warmup_ratio,\n\
            args.train.dyn_bsz_margin,\n\
            args.train.dyn_bsz_buffer_size,\n\
            args.data.drop_last,\n\
            args.data.pin_memory,\n\
            args.data.prefetch_factor")
        import pdb; pdb.set_trace()
        simple_worker_config = WorkerConfig(rank=args.train.local_rank, world_size=args.train.world_size, num_workers=args.data.num_workers)
        ...
        args.train.compute_train_steps(args.data.max_seq_len, args.data.train_size, len(train_ds))
