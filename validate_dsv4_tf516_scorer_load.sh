...
torchrun --nnodes=1 --nproc-per-node=16 --node-rank=0 \
  --master-addr=127.0.0.1 --master-port=64590 \
  ...
  --data.datasets_type=iterable --data.train_sample=16 \
  ...
  --train.global_batch_size=16 --train.micro_batch_size=1 --train.max_steps=1 --train.dyn_bsz=false \
  ...
  --train.checkpoint.output_dir=dsv4_tf516_validate_scorer_load \
  > dsv4_242_tf516_validate_scorer_load_20260920.log 2>&1
